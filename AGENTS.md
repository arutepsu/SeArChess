...
- **Codex / Implementer** → implementation, tests, local refactors within approved boundaries
...
Codex is **not the architect of the system**.
...
Codex must not mix these concerns.
...
Codex may not decide on its own:
...
Codex must obey these strictly.
...
Codex is expected to write tests for the requested behavior.
...
Codex should make the **smallest clean change** that satisfies the task.
...
Codex must be explicit when something appears architecturally questionable or under-specified.

---

## When Codex should stop expanding

Codex should **not** expand the task when encountering:
...
A good Codex contribution in this repo:
