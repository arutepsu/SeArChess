...
class AiServiceRoutes(config: AiServiceConfig):
  ...
  private def handleSuggestion(body: String): IO[Response[IO]] =
    RemoteAiJson.requestFromJson(body) match
      ...
      case Right(request) if request.legalMoves.isEmpty =>
        logWarn("ai_request_rejected", request, "code" -> "BAD_REQUEST", "reason" -> "legalMoves empty")
  ...
  private def logInfo(event: String, request: chess.adapter.ai.remote.RemoteAiMoveSuggestionRequest, fields: (String, Any)*): Unit =
    StructuredLog.info("ai-service", event, (requestFields(request) ++ fields)*)

  private def logWarn(event: String, request: chess.adapter.ai.remote.RemoteAiMoveSuggestionRequest, fields: (String, Any)*): Unit =
    StructuredLog.warn("ai-service", event, (requestFields(request) ++ fields)*)

  private def requestFields(request: chess.adapter.ai.remote.RemoteAiMoveSuggestionRequest): Seq[(String, Any)] =
    Seq(
      "requestId" -> request.requestId,
