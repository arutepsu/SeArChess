...
/** ScalaFX application entry point.
  *
  * `JFXApp3` requires a singleton object; dependencies cannot be passed as constructor arguments
  * because the JavaFX runtime controls when `start()` fires. Use the prepare methods to supply
  * shared dependencies from the composition root before calling `ChessApp.main(args)`.
  *
  ...
  *
  * The command boundary ([[GameSessionCommands]]) and session context must be provided by the
  * composition root (e.g. `game-service/Main`) so that all adapters (GUI, TUI, REST, WebSocket)
  * operate on the same authoritative game identity and persistence layer.
  */
...
object ChessApp extends JFXApp3:
  ...
  /** Register a callback to invoke after [[start()]] has finished setting up the primary stage. The
    * callback runs on the JavaFX application thread, so `Platform.runLater` is safe to use inside
    * it or from any thread it spawns. Called by [[chess.Main]].
    */
