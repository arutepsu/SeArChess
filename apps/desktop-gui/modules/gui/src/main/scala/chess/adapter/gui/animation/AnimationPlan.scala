...
/** Timings for a capture animation sequence.
  *
  * A capture is presented as: Approach -> Attack -> Attack1 -> Dead -> Fade
  */
...
/** Pure data describing a single-piece move animation.
  *
  * The planner captures all required information at planning time using before/after board
  * snapshots, so the renderer never needs to consult chess state.
  *
  * @param movingPiece
  *   colour and type of the piece travelling from → to
  * @param from
  *   source square (empty in the post-move board)
  * @param to
  *   destination square (contains [[movingPiece]] in the post-move board)
  * @param capturedPiece
  *   the piece that occupied [[to]] before the move, if any
  * @param moveDurationMs
  *   total wall-clock duration for a non-capture move
  * @param captureTimings
  *   phase timings used when this plan is a capture
  */
