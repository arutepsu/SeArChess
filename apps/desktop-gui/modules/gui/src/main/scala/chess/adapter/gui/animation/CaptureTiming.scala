...
/** Phase resolver for capture animations.
  ...
  *
  * Capture sequence: Approach -> Attack -> Attack1 -> Dead -> Fade
  */
object CaptureTiming:
  ...
  /** Resolve the active capture phase for the given animation plan and normalized global progress.
    *
    * @param plan
    *   animation plan; expected to represent a capture
  ...
  def resolve(plan: AnimationPlan, globalProgress: Double): PhaseProgress =
    ...
    if elapsed < endApproach then PhaseProgress(Phase.Approach, local(0.0, endApproach))
    else if elapsed < endAttack then PhaseProgress(Phase.Attack, local(endApproach, endAttack))
    else if elapsed < endAttack1 then PhaseProgress(Phase.Attack1, local(endAttack, endAttack1))
    else if elapsed < endDead then PhaseProgress(Phase.Dead, local(endAttack1, endDead))
    else PhaseProgress(Phase.Fade, local(endDead, endFade))
