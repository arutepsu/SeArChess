...
/** Centralized piece-visual rendering helper.
  ...
  * }}}
  *
  * All fallback policy lives here — renderers and scenes never decide what to show when an asset is
  * missing.
  *
  * @param loader
  *   the shared [[SpriteSheetLoader]] instance (provides caching)
  */
class PieceNodeFactory(loader: SpriteSheetLoader, metaRepo: SpriteMetadataRepository):
  ...
  /** Build the visual content node for a piece.
    *
    * Returns an `ImageView` when the sprite asset is available, or a glyph `Text` node as a
    * fallback. The node is not wrapped in a positioned container — callers add it as a child of
    * their own layout pane.
    *
    * Use for: static board squares, promotion buttons.
    *
    * @param id
    *   piece identity + visual state
    * @param squareSize
    *   board square size in logical pixels; governs image fit and glyph font
    * @param frameIndex
    *   zero-based frame to display (0 = first / only frame for static pieces)
    * @param assetKeyOverride
    *   when [[Some]], use this asset key instead of the key derived from [[id]] via
    *   [[VisualResolver]]; used for multi-segment states where the active segment key differs from
    *   the primary key for the state
    * @param flipX
    *   when `true`, mirror the sprite horizontally; used for static piece facing on the board (see
    *   [[chess.adapter.gui.render.PieceFacingPolicy]])
    */
  def content(
      id: PieceVisualId,
      squareSize: Double,
    ...
    metaRepo
      .lookup(resolvedKey)
      .flatMap(meta =>
        loader.loadFrame(resolvedKey, frameIndex, meta).map { image =>
  ...
  /** Build a positioned, opacity-adjusted [[StackPane]] for the animation overlay.
    *
    * Wraps [[content]] in a container with explicit `layoutX`/`layoutY` so it can be placed on the
    * free-floating animation `Pane` over the board grid.
    *
    ...
    *   piece identity + visual state
    * @param x
    *   board-local pixel x of the top-left corner
    * @param y
    *   board-local pixel y of the top-left corner
    * @param squareSize
    *   board square size in logical pixels
    * @param opacity
    *   1.0 = fully opaque; < 1.0 for capture fade-out
    * @param frameIndex
    *   frame to display
    * @param assetKeyOverride
    *   when [[Some]], passed through to [[content]] to select a specific segment asset for
    *   multi-segment states
    * @param flipX
    *   when `true`, mirror the sprite horizontally so the piece faces left; the flip is applied to
    *   the content node and keeps the piece centred within the container
    * @param scale
    *   uniform scale factor applied to the content node; `1.0` = normal; the piece stays centred
    *   inside the container regardless of scale
    */
  def positioned(
      id: PieceVisualId,
      x: Double,
