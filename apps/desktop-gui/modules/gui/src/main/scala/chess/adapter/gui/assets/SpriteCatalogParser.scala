...
/** Pure parser and validator for the sprite catalog JSON file.
  *
  * Transforms a raw JSON string into a validated [[SpriteCatalog]]. No file or classpath I/O is
  * performed here — see [[SpriteCatalogLoader]] for the impure loading wrapper.
  *
  * ===Validation order===
  * Sections are validated with fail-fast chaining:
  *   1. JSON syntax 2. root object structure + `theme` 3. `clipSpecs` — all entries must have valid
  *      `frameCount` and `frameSize` 4. `spriteSheets` — all entries must reference an existing
  *      `clipSpec` 5. `statePlayback` — all entries must have a valid state, mode, and segments
  *      that reference existing `spriteSheets` keys
  *
  * ===No IO===
  * This object has no side effects and no mutable state. All methods are referentially transparent
...
object SpriteCatalogParser:
  ...
  /** Parse and validate a catalog JSON string.
    ...
    *   raw JSON text
    * @return
    *   [[Right]] with a fully validated [[SpriteCatalog]], or [[Left]] with a non-empty list of
    *   [[CatalogValidationError]]s
    */
  def parse(json: String): Either[List[CatalogValidationError], SpriteCatalog] =
    Try(ujson.read(json)).toEither.left
      .map(e => List(CatalogValidationError.ParseError(e.getMessage)))
      .flatMap(parseRoot)
  ...
  private def extractString(
    ...
    obj.get(key) match
      ...
      case Some(ujson.Str(_)) =>
        Left(List(CatalogValidationError.MissingSection(s"$ctx.$key (empty string)")))
  ...
  private def extractObj(
    ...
    obj.get(key) match
      ...
      case Some(_) =>
        Left(List(CatalogValidationError.MissingSection(s"$ctx.$key (not an object)")))
      case None => Left(List(CatalogValidationError.MissingSection(s"$ctx.$key")))
  ...
  private def validateClipSpec(
    ...
    v match
      case ujson.Obj(obj) =>
        for
          frameCount <- extractPositiveInt(
            obj,
            "frameCount",
            key,
            CatalogValidationError.InvalidClipSpec(key, _)
          )
          frameSize <- extractFrameSize(obj, key)
          displaySize <- extractOptionalSize(
            obj,
            "displaySize",
            key,
            CatalogValidationError.InvalidClipSpec(key, _)
          )
          anchor <- extractOptionalPoint(
            obj,
            "anchor",
            key,
            CatalogValidationError.InvalidClipSpec(key, _)
  ...
  private def extractFrameSize(
    ...
    obj.get("frameSize") match
      case Some(ujson.Obj(fs)) =>
        ...
        yield (w, h)
      case Some(_) =>
        Left(CatalogValidationError.InvalidClipSpec(ctx, "frameSize is not an object"))
  ...
  private def validateSpriteSheet(
    ...
    v match
      case ujson.Obj(obj) =>
        for
          path <- extractNonEmptyString(
            obj,
            "path",
            key,
            CatalogValidationError.InvalidSpriteSheet(key, _)
          )
          csKey <- extractNonEmptyString(
            obj,
            "clipSpec",
            key,
            CatalogValidationError.InvalidSpriteSheet(key, _)
          ...
          clipSpec <- clipSpecs
            .get(csKey)
            .toRight(CatalogValidationError.InvalidSpriteSheet(key, s"clipSpec '$csKey' not found"))
  ...
  private def validateStatePlaybackEntry(
    ...
    v match
      case ujson.Obj(obj) =>
        for
          stateStr <- extractNonEmptyString(
            obj,
            "state",
            key,
            CatalogValidationError.InvalidStatePlayback(key, _)
          )
          modeStr <- extractNonEmptyString(
            obj,
            "mode",
            key,
            CatalogValidationError.InvalidStatePlayback(key, _)
    ...
    obj.get("segments") match
      ...
      case Some(ujson.Arr(arr)) =>
        val keys = arr.toSeq.map {
          ...
          case _ =>
            Left(CatalogValidationError.InvalidStatePlayback(ctx, "segment entry is not a string"))
        }
        ...
        if strErrors.nonEmpty then Left(strErrors.head)
          ...
          missing match
            case Some(k) =>
              Left(
                CatalogValidationError.InvalidStatePlayback(
                  ctx,
                  s"segment '$k' not found in spriteSheets"
                )
              )
