 ...
 *
 *  GUI callers pattern-match on these types and never need to import
 *  notation-layer ADTs (NotationFailure, ImportResult, etc.).
 */
...
object GuiNotationOutcome:
   ...
   *
   *  @param state    the domain state produced by the import
   *  @param warnings structured GUI-facing observations collected during import
   *                  (empty when the import was clean)
   */
   ...
   *
   *  @param text the serialised notation string
   */
  final case class ExportSuccess(text: String) extends GuiNotationOutcome

  /** An import or export operation failed.
   *
   *  @param message  primary human-readable description of the problem
   *  @param details  optional supplementary detail (e.g. location within the input)
   *  @param category coarse category enabling the UI to choose an appropriate
   *                  presentation without parsing the message string
   */
 ...
 *
 *  GUI callers may present, filter, or log these without inspecting
 *  notation-layer ADTs.
 *
 *  @param message  human-readable description of the observation
 *  @param category coarse category enabling the UI to decide presentation
 *                  (e.g. show a badge vs. log silently)
 */
...
/** Coarse category of a [[GuiNotationOutcome.Failure]], enabling GUI callers to
 *  present an appropriate message without parsing the failure message string.
 */
enum FailureCategory:
  ...
  /** The notation layer does not support the input dialect or version.
   *  The input may be valid in a variant the system does not implement.
   */
