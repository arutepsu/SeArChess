 ...
 *
 *  This is a thin layout compositor — it owns no application state and makes no
 *  decisions.  The notation sidebar retains its own controller; the history panel
 *  is refreshed by the scene via [[refreshHistory]].
 *
 *  Layout (top to bottom):
 *  1. Notation section  ([[NotationSidebar.root]])
 *  2. Horizontal separator
 *  3. History section   ([[MoveHistoryPanel.root]])
 *
 *  @param notationSidebar  existing notation sidebar; its root is mounted directly
 *  @param historyPanel     move history panel; updated via [[refreshHistory]]
 */
class GameSidePanel(
   ...
   *
   *  Called by the scene on every refresh; the VM is derived from the
   *  controller's current game state, never by this panel.
   */
