 ...
 *
 *  @param squares    all 64 squares in rank-then-file order (rank 0..7, file 0..7)
 *  @param guiState   current state-machine node
 *  @param statusText human-readable status line
 *  @param promotion  Some when the promotion overlay should be visible
 */
