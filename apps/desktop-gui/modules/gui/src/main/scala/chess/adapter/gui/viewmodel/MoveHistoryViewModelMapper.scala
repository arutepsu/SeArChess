 ...
 *
 *  Pairs consecutive half-moves from [[GameState.moveHistory]] into rows.
 *  The first half-move of each pair occupies the white column; the second
 *  occupies the black column.  When the history has an odd number of
 *  half-moves, the last row's black column is [[None]].
 *
 *  Move notation format: coordinate notation "e2-e4", with promotion suffix
 *  "=Q" / "=R" / "=B" / "=N" when applicable.
 */
