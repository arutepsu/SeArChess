...
class CaptureTimingSpec extends AnyFlatSpec with Matchers:
  ...
  private val dummyPlan = AnimationPlan(
    ...
    from = Position.from(0, 0).getOrElse(scala.sys.error("bad pos")),
    to = Position.from(1, 1).getOrElse(scala.sys.error("bad pos")),
  ...
  private val deadEnd =
    (timings.approachMs + timings.attackMs + timings.attack1Ms + timings.deadMs) / total
  ...
  it should "transition from Approach to Attack at approachEnd" in {
    CaptureTiming
      .resolve(dummyPlan, approachEnd - 0.001)
      .phase shouldBe CaptureTiming.Phase.Approach
  ...
  it should "return localProgress=0.5 at the midpoint of each phase" in {
    ...
    CaptureTiming
      .resolve(dummyPlan, (approachEnd + attackEnd) / 2)
      .localProgress shouldBe 0.5 +- 1e-10
    CaptureTiming
      .resolve(dummyPlan, (attackEnd + attack1End) / 2)
      .localProgress shouldBe 0.5 +- 1e-10
