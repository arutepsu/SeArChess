...
class GuiNotationApiExtraSpec extends AnyFlatSpec with Matchers {
  val api = new GuiNotationApi(new NotationFacade[GameState] {
    ...
    override def executeImport(parsed: ParsedNotation, target: ImportTarget) = Left(
      ImportFailure.IncompatibleTarget(ParsedNotationKind.Fen, ImportTarget.PositionTarget, "fail")
    )
    override def executeExport(data: GameState, format: NotationFormat) = Left(
  ...
  it should "handle import failure as semantic error" in {
    ...
    result shouldBe a[GuiNotationOutcome.Failure]
    result
      .asInstanceOf[GuiNotationOutcome.Failure]
      .category should not be FailureCategory.UnavailableFeature
  }
