...
/** Integration spec for the Scala RemoteAiMoveSuggestionClient against a live AI provider.
  ...
  * The tests are skipped automatically when the service is not reachable so they do not break CI.
  *
...
class RemoteAiIntegrationSpec extends AnyFlatSpec with Matchers with EitherValues:
  ...
  private def context(
    ...
    AIRequestContext.fromSession(
      GameSession.create(
        ...
        SideController.AI(Some("stub-model")),
  ...
  it should "round-trip a promotion move through the provider" in {
    ...
    List("queen", "rook", "bishop", "knight") should contain(
      promo.getOrElse(fail("promotion was None after shouldBe defined"))
    )
  }
