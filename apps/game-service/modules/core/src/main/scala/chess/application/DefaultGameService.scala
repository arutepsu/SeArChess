 ...
 *
 *  Thin orchestration facade: delegates every command and query to the
 *  appropriate existing application service.  Business logic and event policy
 *  stay inside [[chess.application.session.service.SessionGameService]] and
 *  [[chess.application.session.service.SessionService]]; this class only routes
 *  calls and maps errors for the transport boundary.
 *
 *  === Dependency map ===
 ...
 *  - [[aiService]]      — optional AI move orchestration; [[AITurnError.NotConfigured]]
 *                         is returned when `None`
 *
 *  === Note: exhaustive match coupling from GameStatus.Resigned ===
 *  Adding [[chess.domain.model.GameStatus.Resigned]] required updates to many
 *  exhaustive match sites in adapters, mappers, and notation modules.  This
 *  is a symptom of the sealed enum being matched directly throughout the stack
 *  rather than going through a dedicated mapper boundary.  A future improvement
 *  would route all `GameStatus` translation through a single presentation mapper
 *  so that new statuses only require one touch point.  Not addressed here.
 *
 *  @param commands       session command boundary (newGame, submitMove, resignGame)
 *  @param sessionService session lifecycle, queries, cancel, list
 *  @param gameRepository read-only game state port
 *  @param publisher      fire-and-forget event publisher for [[chess.application.event.AppEvent.MoveRejected]]
 *  @param aiService      optional AI turn orchestration service
 */
class DefaultGameService(
   ...
   *
   *  [[AppEvent.MoveRejected]] is an '''interaction/audit event''', not a state-
   *  transition event: it records that a move was attempted and rejected, but the
   *  game state does not change.  It is published here — at the facade level —
   *  because [[chess.application.session.service.SessionGameService]] only sees
   *  successful paths and cannot observe rejections.  The facade is the only place
   *  that holds both the session identity (needed for the event) and the domain
   *  rejection error.
   *
   *  Only [[SessionMoveError.DomainRejection]] triggers [[AppEvent.MoveRejected]].
   *  Infrastructure failures (persistence errors, session not found) and
   *  unauthorized-controller rejections are caller errors, not game events, and
   *  are returned as-is without publishing.
   */
  def submitMove(
    gameId:     GameId,
    move:       Move,
    ...
    val result =
      for
        ...
        state   <- stateResult.left.map(e => SessionMoveError.PersistenceFailed(
  ...
  def resignGame(
    ...
    for
      ...
      state   <- gameRepository.load(session.gameId)
   ...
   *
   *  Session cancellation is an '''administrative termination''', not a game
  ...
  def triggerAIMove(
    ...
    aiService match
      ...
      case Some(ai) =>
        for
          session <- sessionService.getSession(sessionId)
                       .left.map(AITurnError.SessionLookupFailed(_))
          state   <- gameRepository.load(session.gameId)
                       .left.map(AITurnError.GameStateLookupFailed(_))
          result  <- ai.requestAIMove(session, state, now)
        yield result

  /** Resolves session from [[gameId]] then delegates to [[triggerAIMove]].
   *
   *  REST adapters carry game IDs in the URL; calling this avoids a redundant
   *  session lookup that would otherwise happen once in the route (to convert
   *  game ID to session ID) and once inside [[triggerAIMove]] (to reload the
   *  session).  The service boundary owns the mapping; the route stays thin.
   */
