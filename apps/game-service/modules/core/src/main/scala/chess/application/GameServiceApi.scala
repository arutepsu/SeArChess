 ...
 *
 *  This trait is the single entry point for all inbound adapters (REST, WebSocket
 *  command channel, future gRPC, etc.) that need to interact with the game-session
 *  capability.  It intentionally hides the internal coordination between
 *  [[chess.application.session.service.SessionGameService]],
 *  [[chess.application.session.service.SessionService]], and the underlying
 *  repository/event ports from transport adapters.
 *
 *  === Command/query ownership ===
 *  Commands (state-mutating operations) load their own session and game state
 *  from the given identifiers.  This keeps transport adapters thin: they parse
 *  the request, call one method, and map the result to a response.
 *
 *  Queries return current state without side effects.
 *
 *  === Extraction boundary ===
 *  When the Game Service is extracted to a standalone process this trait
 *  becomes the service contract.  A remote implementation (HTTP client,
 *  gRPC stub) can replace [[DefaultGameService]] without touching any adapter.
 *
...
trait GameServiceApi:
   ...
   *
   *  Atomically persists session metadata and the initial [[GameState]], then
   *  publishes [[chess.application.event.AppEvent.SessionCreated]].
   */
  def createGame(
    mode:            SessionMode,
   ...
   *
   *  Loads the session and game state, validates the move against session
   *  policy and domain rules, then persists atomically.  On domain rejection,
   *  publishes [[chess.application.event.AppEvent.MoveRejected]].  On success,
   *  publishes [[chess.application.event.AppEvent.MoveApplied]] (and
   ...
   *
   *  Records [[chess.domain.model.GameStatus.Resigned]] on the game state,
   *  advances the session to [[chess.application.session.model.SessionLifecycle.Finished]],
   *  and persists atomically.  Publishes [[chess.application.event.AppEvent.GameResigned]].
   *
   ...
   *  [[chess.domain.state.GameState]] is intentionally left unchanged: no winner
   *  is recorded and no [[chess.domain.model.GameStatus]] transition occurs.  Only
   *  the session lifecycle advances to
   ...
   *
   *  Publishes [[chess.application.event.AppEvent.SessionCancelled]].
   ...
   *
   *  This query is part of the Game Service application boundary. Transport
   *  adapters map the returned [[LegalMovesView]] into their own DTOs.
   */
