...
/** Concrete implementation of [[GameSessionCommands]] and the composition root for the game-session
  * capability inside the modular monolith.
  *
  * This class has three distinct roles, each occupying its own section:
  *
  * ===1. Game-session command surface ([[GameSessionCommands]])===
  * [[newGame]] and [[submitMove]] are the authoritative write operations. Both operations:
  ...
  *   - emit post-persistence [[chess.application.event.AppEvent]]s These are the operations the
  *     future extracted game-session command service would own. Adapters that depend only on
  *     [[GameSessionCommands]] are already extraction-ready.
  *
  * ===2. Session lifecycle management===
  * [[createSession]], [[preparePromotion]], and [[updateLifecycle]] mutate session state only (not
  * game state) and delegate to [[SessionService]]. They write only to
  * [[chess.application.port.repository.SessionRepository]] — not [[SessionGameStore]]. These
  * operations are used by desktop adapters (GUI, desktop-session import) that manage session
  * context locally.
  *
  * ===3. Query delegation===
  * [[getSession]] and [[getSessionByGameId]] are pure reads forwarded to [[SessionService]].
  * Callers that only need reads may depend on [[SessionService]] directly; these delegating methods
  * exist so that adapters that need both reads and commands can use a single dependency. In a
  * future extraction, these reads could remain on the command service (if it is the read authority)
  * or be served by a separate read model.
  *
