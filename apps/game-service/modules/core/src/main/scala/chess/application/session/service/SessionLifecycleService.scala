...
/** Lifecycle and session ownership service.
  *
  * This service does not own full game-state persistence and is not the normal gameplay command
  * orchestrator.
  *
  * Responsibilities:
  ...
  *   - provide a session-aware move entry point that checks controller ownership before delegating
  *     to the chess engine
...
class SessionLifecycleService(
  ...
  /** Create a new [[GameSession]] in the [[SessionLifecycle.Created]] phase and persist it
    * immediately.
    *
    * Publishes [[AppEvent.SessionCreated]] on success.
    *
    * The caller must supply a [[GameId]] obtained from wherever the underlying game state is
    * initialised (e.g. `GameStateCommandService.createNewGame`).
    */
  def createSession(
      gameId: GameId,
      mode: SessionMode,
    ...
    save(session).map { saved =>
      publisher.publish(
        AppEvent.SessionCreated(
          ...
          saved.gameId,
          saved.mode,
          saved.whiteController,
          saved.blackController
        )
      )
      saved
    }
  ...
  /** Transition the session to a new [[SessionLifecycle]] phase and persist the change.
    *
    * The transition is validated by [[SessionLifecyclePolicy]] before any persistence call is made.
    * Invalid transitions return [[SessionError.InvalidLifecycleTransition]] immediately.
    *
  ...
  def updateLifecycle(
    ...
    for
      ...
      _ <- SessionLifecyclePolicy
        .validateTransition(session.lifecycle, next)
        .left
        .map(SessionError.InvalidLifecycleTransition(_))
  ...
  /** Signal that the session is paused waiting for a promotion piece choice.
    *
    * Call this when the application detects that the current player's pawn has reached the back
    * rank and no promotion piece has been chosen yet. Transitions the session from
    * [[SessionLifecycle.Active]] to [[SessionLifecycle.AwaitingPromotion]] and persists the change.
    *
  ...
  /** Retrieve the session associated with a given [[GameId]].
    *
    * Useful when the caller knows the game id (e.g. from a REST path segment) but has not yet
    * resolved the session id. Delegates to [[SessionRepository.loadByGameId]].
    *
    * Returns [[SessionError.GameSessionNotFound]] when no session references that game, keeping the
    * "not found" distinction clean for callers.
    */
  ...
  /** Cancel a session by advancing its lifecycle to [[SessionLifecycle.Cancelled]].
    *
    * Valid for sessions in [[SessionLifecycle.Created]] or [[SessionLifecycle.Active]]. The
    * underlying game state is left unchanged (no winner is recorded). Publishes
    * [[chess.application.event.AppEvent.SessionCancelled]] on success.
    *
    * In SQLite mode, the session row update and the outbox payload are committed in the same JDBC
    * transaction via [[SessionRepository.saveCancelWithOutbox]].
    *
    * Returns [[SessionError.InvalidLifecycleTransition]] when the session is already closed.
    */
  def cancelSession(
      sessionId: SessionId,
      now: Instant = Instant.now()
    ...
    for
      ...
      _ <- SessionLifecyclePolicy
        .validateTransition(session.lifecycle, SessionLifecycle.Cancelled)
        .left
        .map(SessionError.InvalidLifecycleTransition(_))
      ...
      _ <- repository
        .saveCancelWithOutbox(updated, payload)
        .left
  ...
  /** Session-aware move entry point (pure computation).
    *
    * Performs session-level ownership and lifecycle checks before delegating to the chess engine.
    * Returns the new [[GameState]] and an updated [[GameSession]] (with the correct lifecycle)
    * without touching any repository or publishing any events.
    *
    * The caller ([[chess.application.session.service.SessionGameCommandService.submitMove]]) is
    * responsible for:
    *   1. Persisting both the updated [[GameSession]] and the new [[GameState]] via
    *      [[chess.application.port.repository.SessionGameStore]]. 2. Publishing
    *      [[chess.application.event.AppEvent]]s after persistence succeeds.
    *
    * Keeping this method pure eliminates the partial-failure window that existed when session
    * persistence and event publication were interleaved with the separate game-state save.
    *
    * Flow:
    *   1. Reject immediately if session is [[SessionLifecycle.Finished]]. 2. Check
    *      [[ActorControlPolicy.canAct]] — reject if the requesting controller does not own the side
    *      to move. 3. Delegate to [[GameStateCommandService.applyMove]] for chess legality. 4. Compute the
    *      correct next lifecycle phase (no repository write). 5. Return the updated domain state
    *      and the (possibly updated) session.
    *
    * @param session
    *   current session (loaded by caller; not re-fetched)
    * @param state
    *   current chess state
    * @param move
    *   the move to apply
    * @param requestingController
    *   the controller that is submitting the move
    * @param now
    *   wall-clock instant for lifecycle timestamps
    * @return
    *   updated (GameState, GameSession) or a [[SessionMoveError]]
    */
  def applyMove(
      session: GameSession,
      state: GameState,
    ...
    for
      ...
      nextState <- GameStateCommandService.applyMove(state, move).left.map(SessionMoveError.DomainRejection(_))
  ...
  private def rejectIfFinished(session: GameSession): Either[SessionMoveError, Unit] =
    ...
    then Left(SessionMoveError.SessionFinished)
    else Right(())
  ...
  /** After a successful move, compute the correct next lifecycle phase (pure).
    *
    * Returns an updated [[GameSession]] with the new lifecycle applied but does NOT write to any
    * repository. The caller is responsible for persisting.
    *
    * Transition table:
    *   - Any lifecycle + terminal game (checkmate/draw) → [[SessionLifecycle.Finished]]
    *   - [[SessionLifecycle.Created]] + non-terminal → [[SessionLifecycle.Active]] (first move
    ...
    nextLifecycle match
      ...
      case Some(next) =>
        SessionLifecyclePolicy
          ...
          .left
          .map(msg =>
            SessionMoveError.PersistenceFailed(SessionError.InvalidLifecycleTransition(msg))
          )
