 ...
 *  [[saveTerminal]] extends the atomicity guarantee to include pre-serialised
 *  outbox payloads.  When SQLite is the backing store, all three writes — session
 *  row, game-state row, and outbox rows — land in the same JDBC transaction.
 *  If any write fails the entire transaction is rolled back.
 *
 *  In-memory and other non-SQLite implementations inherit the default
 *  implementation of [[saveTerminal]], which delegates to [[save]] and ignores
 *  the payload list (no durable outbox exists in those modes).
 *
...
trait SessionGameStore:
   ...
   *  `outboxPayloads`.  This is correct for in-memory persistence and any mode
   *  where no durable outbox is configured: the caller supplies a no-op
   *  serializer so `outboxPayloads` is always empty, and [[save]] suffices.
   *
   *  SQLite implementations override this to include all three writes in a
   *  single JDBC transaction: if the outbox insert fails the session and game
   *  state writes are also rolled back, closing the consistency gap.
   *
   *  @param session        the updated session metadata
   *  @param state          the new game state
   *  @param outboxPayloads pre-serialised JSON strings to insert into the outbox;
   *                        empty when no durable outbox is configured
   */
  def saveTerminal(
    session:        GameSession,
    state:          GameState,
