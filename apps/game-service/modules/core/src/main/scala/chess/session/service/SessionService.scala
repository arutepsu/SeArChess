...
class SessionService(
  ...
  /** Create a new [[GameSession]] in the [[SessionLifecycle.Created]] phase and persist it
    ...
    * initialised (e.g. `ChessService.createNewGame`).
    */
  def createSession(
      gameId: GameId,
      mode: SessionMode,
  ...
  /** Session-aware move entry point (pure computation).
    ...
    *
    * The caller ([[chess.application.session.service.SessionGameService.submitMove]]) is
    * responsible for:
    ...
    *      to move. 3. Delegate to [[ChessService.applyMove]] for chess legality. 4. Compute the
  ...
  def applyMove(
    ...
    for
      ...
      nextState <- ChessService.applyMove(state, move).left.map(SessionMoveError.DomainRejection(_))
