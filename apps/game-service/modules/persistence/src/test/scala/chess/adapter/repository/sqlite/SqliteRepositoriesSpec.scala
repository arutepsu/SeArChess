...
/** Tests for SQLite-backed repositories.
  *
  * Each test uses a private temp file so tests are fully isolated. Restart survival tests open two
  * separate [[SqliteDataSource]] instances against the same file path to simulate a JVM restart.
  */
