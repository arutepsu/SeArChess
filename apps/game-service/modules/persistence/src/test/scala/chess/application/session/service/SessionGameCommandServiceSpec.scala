...
/** Verifies that [[SessionGameCommandService]] publishes events only after the combined session +
  * game-state write has succeeded, and that both writes are visible in the repositories when events
  * fire.
  */
class SessionGameCommandServiceSpec extends AnyFlatSpec with Matchers with EitherValues with OptionValues:
  ...
  private def freshFixture(collector: CollectingEventPublisher = CollectingEventPublisher()) =
    ...
    val svc = new SessionGameCommandService(service, store, collector)
    (svc, service, sessionRepo, gameRepo, collector)

  private def createSession(lifecycleSvc: SessionLifecycleService) =
    lifecycleSvc
      .createSession(
  ...
  /** Minimal one-move-to-checkmate position (same layout as SessionLifecycleServiceEventSpec). */
  ...
  "SessionGameCommandService.submitMove" should "publish MoveApplied after a successful move" in {
    val (svc, service, _, _, collector) = freshFixture()
  ...
  it should "call store.save before publishing MoveApplied" in {
    ...
    val service = new SessionLifecycleService(sessionRepo, _ => ())
    val svc = new SessionGameCommandService(service, countingStore, orderingPublisher)
    val session = service
      .createSession(
        GameId.random(),
  ...
  it should "write the updated session and game state together through SessionGameStore" in {
    ...
    val service = new SessionLifecycleService(new InMemorySessionRepository, _ => ())
    val svc = new SessionGameCommandService(service, store, collector)
    ...
    val (nextState, nextSession) =
      svc.submitMove(session, state, move, SideController.HumanLocal).value
  ...
  it should "surface a combined write failure and publish no events" in {
    ...
    val err = svc
      .submitMove(session, GameStateFactory.initial(), move, SideController.HumanLocal)
      .left
      .value
  ...
  it should "persist the updated game state to the game repository" in {
    val (svc, service, _, gameRepo, _) = freshFixture()
    val session = createSession(service)
    val state = GameStateFactory.initial()
    val move = Move(Position.from(4, 1).value, Position.from(4, 3).value)
    svc.submitMove(session, state, move, SideController.HumanLocal)
  ...
  it should "persist the updated session lifecycle to the session repository" in {
    val (svc, service, sessionRepo, _, _) = freshFixture()
    val session = createSession(service)
    val state = GameStateFactory.initial()
    val move = Move(Position.from(4, 1).value, Position.from(4, 3).value)
    svc.submitMove(session, state, move, SideController.HumanLocal)
  ...
  "SessionGameCommandService.newGame" should "publish SessionCreated on success" in {
    val (svc, _, _, _, collector) = freshFixture()
    val result =
      svc.newGame(SessionMode.HumanVsHuman, SideController.HumanLocal, SideController.HumanLocal)
  ...
  it should "persist the initial game state to the game repository" in {
    val (svc, _, _, gameRepo, _) = freshFixture()
    val (_, session) = svc
      .newGame(
        SessionMode.HumanVsHuman,
        SideController.HumanLocal,
  ...
  it should "persist the session to the session repository" in {
    val (svc, _, sessionRepo, _, _) = freshFixture()
    val (_, session) = svc
      .newGame(
        SessionMode.HumanVsHuman,
        SideController.HumanLocal,
  ...
  "SessionGameCommandService.resignGame" should "write the resigned state and finished session through SessionGameStore" in {
    val store = new RecordingStore(Right(()))
    val collector = CollectingEventPublisher()
    val service = new SessionLifecycleService(new InMemorySessionRepository, _ => ())
    val svc = new SessionGameCommandService(service, store, collector)
