...
/** Maps a [[GameView]] to a [[GameResponse]] DTO.
  *
  * Derives all transport fields from the application read model; no domain rules are invoked here.
  * Legal-move derivation is pre-computed in [[GameView.fromState]] and carried through
  * [[GameView.legalMoves]].
  */
object GameMapper:
  ...
  /** Build a [[GameResponse]] from a [[GameView]].
    *
    * @param view
    *   application read model for the game position
    * @param promotionPending
    *   forwarded directly into the DTO; callers set this when a promotion choice is required before
    *   play can continue. Defaults to `false` (never true in the current REST v1 error-driven
    *   promotion flow).
    */
  def toGameResponse(view: GameView, promotionPending: Boolean = false): GameResponse =
    ...
    val board = view.board.map { case (pos, piece) =>
      PieceDto(
        square = pos.toString,
