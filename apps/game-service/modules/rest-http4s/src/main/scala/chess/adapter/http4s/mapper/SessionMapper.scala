 ...
 *
 *  REST v1 does not accept the literal "AI" as an inbound controller value.
 *  AI controllers are derived from [[SessionMode]] by [[resolveCreateControllers]].
 */
object SessionMapper:
  ...
  /** Parse a human [[SideController]] from an optional request string.
   *  Absent or "HumanLocal" -> [[SideController.HumanLocal]] (default).
   *
   *  "AI" is intentionally not accepted here. REST v1 derives AI-controlled
   *  seats from [[SessionMode]] in [[resolveCreateControllers]].
   */
   ...
   *
   *  Rule: mode determines AI seats server-side. Controller fields are accepted
   *  only for human-controlled seats; clients cannot request AI engine identity
   *  or override an AI seat through the REST contract.
   */
    ...
    mode match
      ...
      case SessionMode.HumanVsAI =>
        black match
          ...
          case None =>
            parseController(white).map(whiteController =>
              (whiteController, SideController.AI()))

      case SessionMode.AIVsAI =>
        if white.isDefined || black.isDefined then
          Left("Controllers are server-controlled AI for AIVsAI; omit whiteController and blackController")
        else
          Right((SideController.AI(), SideController.AI()))
  ...
  def parseSide(s: String): Either[String, Color] =
    Color.values.find(_.toString == s)
