...
/** In-memory tests for [[Http4sGameRoutes]].
  *
  * All game-route behaviours are exercised without a network socket. The fixture wires routes
  * through [[DefaultGameService]] (the [[chess.application.GameServiceApi]] implementation),
  * replacing the previous three-dependency split.
  */
class Http4sGameRoutesSpec extends AnyFlatSpec with Matchers:
  ...
  it should "return 422 AI_MOVE_REJECTED when AI provider returns malformed move data" in {
    ...
    val resp =
      run(gameRoutes, Request[IO](Method.POST, Uri.unsafeFromString(s"/games/$gameId/ai-move")))
  ...
  "GET /games/{gameId}/replay?ply={n}" should "return reconstructed state and raw moves" in {
    val (gameRoutes, sessRoutes) = makeRoutes()
    val gameId = createSession(sessRoutes)

    val move1 = Request[IO](Method.POST, Uri.unsafeFromString(s"/games/$gameId/moves"))
      .withBodyStream(jsonBody("""{"from":"e2","to":"e4"}"""))
    run(gameRoutes, move1).status shouldBe Status.Ok

    val move2 = Request[IO](Method.POST, Uri.unsafeFromString(s"/games/$gameId/moves"))
      .withBodyStream(jsonBody("""{"from":"e7","to":"e5"}"""))
    run(gameRoutes, move2).status shouldBe Status.Ok

    val replayResp =
      run(gameRoutes, Request[IO](Method.GET, Uri.unsafeFromString(s"/games/$gameId/replay?ply=1")))

    replayResp.status shouldBe Status.Ok
    val json = bodyJson(replayResp)
    json("ply").num.toInt shouldBe 1
    json("totalPlies").num.toInt shouldBe 2
    json("rawMoves").arr should have size 2
    json("game")("moveHistory").arr should have size 1
    json("game")("currentPlayer").str shouldBe "Black"
  }

  it should "return 400 INVALID_PLY when replay ply is out of range" in {
    val (gameRoutes, sessRoutes) = makeRoutes()
    val gameId = createSession(sessRoutes)
    ...
    val replayResp =
      run(
        gameRoutes,
        Request[IO](Method.GET, Uri.unsafeFromString(s"/games/$gameId/replay?ply=99"))
      )

    replayResp.status shouldBe Status.BadRequest
    val json = bodyJson(replayResp)
    json("code").str shouldBe "INVALID_PLY"
  }
  ...
  "GET /games/{gameId}/legal-moves" should "return legal moves for the current player" in {
    ...
    val resp =
      run(gameRoutes, Request[IO](Method.GET, Uri.unsafeFromString(s"/games/$gameId/legal-moves")))
    resp.status shouldBe Status.Ok
    val json = bodyJson(resp)
  ...
  it should "return 404 for legal moves of an unknown game id" in {
    ...
    val unknown = UUID.randomUUID().toString
    val resp =
      run(gameRoutes, Request[IO](Method.GET, Uri.unsafeFromString(s"/games/$unknown/legal-moves")))
    resp.status shouldBe Status.NotFound
    bodyJson(resp)("code").str shouldBe "GAME_NOT_FOUND"
  }
  ...
  it should "return PROMOTION_REQUIRED (422) when a pawn reaches the 8th rank without a promotion piece" in {
    ...
    val pos =
      (alg: String) => Position.fromAlgebraic(alg).getOrElse(throw AssertionError(s"bad: $alg"))
    ...
    val promotionState = GameStateFactory
      .initial()
      .copy(
  ...
  "GET /games/{gameId} (storage failure)" should "return 500 INTERNAL_ERROR when game repository fails" in {
    ...
    val gameService =
      DefaultGameService(svc, sessionLifecycleService, FailingGameRepository(), _ => ())
    val failingRoutes = Http4sGameRoutes(gameService).routes.orNotFound

    val resp = run(
      failingRoutes,
      Request[IO](Method.GET, Uri.unsafeFromString(s"/games/${UUID.randomUUID()}"))
    )
  ...
  it should "return 500 INTERNAL_ERROR when session store save fails after a valid move" in {
    ...
    val failingSvc =
      SessionGameCommandService(sessionLifecycleService, FailingSessionGameStore(), _ => ())
  ...
  it should "return 409 GAME_ALREADY_FINISHED when resigning an already-finished game" in {
    ...
    run(
      gameRoutes,
      Request[IO](Method.POST, Uri.unsafeFromString(s"/games/$gameId/resign"))
        .withBodyStream(jsonBody("""{"side":"White"}"""))
    )
  ...
  it should "return 404 for an unknown game id" in {
    ...
    val resp = run(
      gameRoutes,
      Request[IO](Method.POST, Uri.unsafeFromString(s"/games/${UUID.randomUUID()}/ai-move"))
    )
    resp.status shouldBe Status.NotFound
    bodyJson(resp)("code").str shouldBe "GAME_NOT_FOUND"
  }
