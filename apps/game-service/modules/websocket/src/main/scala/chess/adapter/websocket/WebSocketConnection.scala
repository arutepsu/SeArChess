 ...
 *
 *  Separates the connection registry and event publisher from the concrete
 *  Java-WebSocket library so that tests can supply stub implementations
 ...
 *
 *  === Contract ===
 *  `send` must not throw — implementations should absorb transient errors
 *  and the publisher wraps calls in a try-catch anyway.
 */
