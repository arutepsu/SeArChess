...
/** [[EventPublisher]] adapter that routes [[AppEvent]]s to WebSocket clients.
  ...
  *   1. The event is mapped to a JSON string by [[WebSocketMessageMapper]]. 2. All connections
  *      subscribed to `event.gameId` in the [[WebSocketConnectionRegistry]] receive the message.
  *
  * Individual send failures are swallowed — a broken connection must not interrupt delivery to
  * other subscribers or propagate an exception back to the service caller.
  *
  ...
  *
  * @param registry
  *   live connection registry shared with [[ChessWebSocketServer]]
  */
