...
import chess.server.http.{
  CorsMiddleware,
  HealthRoutes,
  HistoryOutboxOpsRoutes,
  HttpMetricsMiddleware,
  HttpMetricsRegistry,
  HttpRequestLoggingMiddleware,
  MigrationAdminRoutes
}
...
object ServerWiring:
  ...
  def start(config: AppConfig): (AppContext, ServerRuntime) =
    ...
    val baseOpsRoutes =
      HealthRoutes.routes <+>
        MetricsRoutes.routes(metricsRegistry, domainMetrics) <+>
        HistoryOutboxOpsRoutes(events.historyOutbox).routes

    val internalOpsRoutes =
      if config.migrationAdminEnabled then
        val token = config.migrationAdminToken.getOrElse(
          throw RuntimeException(
            "migrationAdminToken must be set when migrationAdminEnabled - config validation should prevent this state"
          )
        )
