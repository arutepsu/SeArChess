...
/** Errors that [[ArchiveOnTerminalEventHandler.handle]] may return.
  *
  * ===Retryability===
  *   - [[SnapshotNotClosed]] — retryable; the session lifecycle has not yet reached `Finished`.
  *     This is a race between the terminal event and the session close write; the handler should
  *     back off and retry.
  ...
  *   - [[MaterializationFailed]] — not retryable without code change; the notation engine returned
  *     an unexpected failure for this state.
