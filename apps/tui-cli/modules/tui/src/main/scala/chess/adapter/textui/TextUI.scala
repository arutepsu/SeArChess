...
/** Text-based UI for the chess game.
  ...
  * the unified application mutation boundary — which validates the move, persists the new
  * [[GameState]], persists the updated session lifecycle, and publishes
  * [[chess.application.event.AppEvent]]s. [[GameStateObservable]] is then updated as a notification
  * bridge so other adapters (e.g. the GUI) observe the state change.
  *
  * The two paths are explicitly separated inside [[loop]] via pattern-matching on `(commands,
  * sessionContext)`. No mutation logic is shared between them.
  *
  * @param console
  *   I/O surface for reading commands and writing output
  * @param game
  *   cross-adapter state notification bridge; always updated after a successful move so other
  *   adapters observe it
  * @param commands
  *   unified application mutation boundary; supply for session-aware mode, omit for local mode
  * @param sessionContext
  *   shared [[DesktopSessionContext]] holding the current session; must wrap a session already
  *   persisted in the repository backing `commands`
  */
final class TextUI(
    console: Console,
      ...
      case Right(TextUiCommand.New) =>
        (commands, sessionContext) match
          case (Some(cmds), Some(ctx)) =>
            ...
            cmds.newGame(
              ctx.getSession.mode,
              ctx.getSession.whiteController,
              ctx.getSession.blackController
            ) match
            ...
            (commands, sessionContext) match
              case (Some(cmds), Some(ctx)) =>
                ...
                cmds.submitMove(
                  ctx.getSession,
                  state,
                  Move(pm.from, pm.to, Some(pieceType)),
                  SideController.HumanLocal
                ) match
                  case Left(err) =>
                    console.printLine(renderSessionMoveError(err))
  ...
  /** Translate a [[SessionMoveError]] to a user-facing string.
    *
    * Domain rejections delegate to [[ConsoleRenderer.renderApplicationError]] so the rendering
    * vocabulary is consistent between local and session mode. Session-level rejections produce
    * dedicated messages.
    */
  private def renderSessionMoveError(err: SessionMoveError): String = err match
    ...
    case SessionMoveError.SessionFinished =>
      "The game is already finished. Start a new game with 'new'."
