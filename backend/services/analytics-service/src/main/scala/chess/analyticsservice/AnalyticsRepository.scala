...
trait AnalyticsRepository:
  ...
  def getHeadToHead(runId: String): Either[String, List[HeadToHeadRow]]
  def getFamilyMatchups(runId: String): Either[String, List[FamilyMatchupRow]]
  def getGameResults(runId: String): Either[String, List[GameResultRow]]
