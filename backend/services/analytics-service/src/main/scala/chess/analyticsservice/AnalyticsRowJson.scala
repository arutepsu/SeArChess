...
object AnalyticsRowJson:
  ...
  def headToHead(r: HeadToHeadRow): Value =
    Obj(
      "whiteBotId"  -> r.whiteBotId,
      "blackBotId"  -> r.blackBotId,
      "gamesPlayed" -> r.gamesPlayed.toDouble,
  ...
  def familyMatchup(r: FamilyMatchupRow): Value =
    Obj(
      "whiteBotFamily" -> r.whiteBotFamily,
      "blackBotFamily" -> r.blackBotFamily,
      "games"          -> r.games.toDouble,
      "whiteScore"     -> r.whiteScore,
      "blackScore"     -> r.blackScore,
      "draws"          -> r.draws.toDouble
  ...
  def gameResult(r: GameResultRow): Value =
    val obj = Obj(
      "gameId"            -> r.gameId,
      ...
      "blackBotId"        -> r.blackBotId,
      "result"            -> r.result,
      "winnerBotId"       -> r.winnerBotId.map(ujson.Str(_)).getOrElse(ujson.Null),
      "loserBotId"        -> r.loserBotId.map(ujson.Str(_)).getOrElse(ujson.Null),
      "terminationReason" -> r.terminationReason,
      "totalPly"          -> r.totalPly.toDouble,
      "durationMillis"    -> r.durationMillis.toDouble,
      "endedAt"           -> r.endedAt
    )
    obj
