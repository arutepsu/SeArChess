...
final class SlickAnalyticsRepository(db: Database, schema: String) extends AnalyticsRepository:
  ...
  private given GetResult[HeadToHeadRow] = GetResult { r =>
    HeadToHeadRow(r.nextString(), r.nextString(), r.nextLong(), r.nextLong(),
      r.nextLong(), r.nextLong(), r.nextDouble(), r.nextDouble())
  }

  private given GetResult[FamilyMatchupRow] = GetResult { r =>
    FamilyMatchupRow(r.nextString(), r.nextString(), r.nextLong(), r.nextDouble(), r.nextDouble(), r.nextLong())
  }

  private given GetResult[GameResultRow] = GetResult { r =>
    GameResultRow(
      gameId            = r.nextString(),
      tournamentId      = r.nextString(),
      whiteBotId        = r.nextString(),
      blackBotId        = r.nextString(),
      result            = r.nextString(),
      winnerBotId       = r.nextStringOption(),
      loserBotId        = r.nextStringOption(),
      terminationReason = r.nextString(),
      totalPly          = r.nextLong(),
      durationMillis    = r.nextLong(),
      endedAt           = r.nextString()
    )
  }
  ...
  override def getHeadToHead(runId: String): Either[String, List[HeadToHeadRow]] =
    run(
      sql"""SELECT "whiteBotId", "blackBotId", "gamesPlayed", "whiteWins", "blackWins", "draws", "whiteScore", "blackScore"
            FROM #$t.analytics_head_to_head
            WHERE run_id = $runId
            ORDER BY "whiteBotId", "blackBotId"""".as[HeadToHeadRow]
    )

  override def getFamilyMatchups(runId: String): Either[String, List[FamilyMatchupRow]] =
    run(
      sql"""SELECT "whiteBotFamily", "blackBotFamily", "games", "whiteScore", "blackScore", "draws"
            FROM #$t.analytics_family_matchups
            WHERE run_id = $runId
            ORDER BY "whiteBotFamily", "blackBotFamily"""".as[FamilyMatchupRow]
    )

  override def getGameResults(runId: String): Either[String, List[GameResultRow]] =
    run(
      sql"""SELECT "gameId", "tournamentId", "whiteBotId", "blackBotId", "result",
                   "winnerBotId", "loserBotId", "terminationReason", "totalPly", "durationMillis", "timestamp"
            FROM #$t.analytics_game_results
            WHERE run_id = $runId
            ORDER BY "timestamp" ASC""".as[GameResultRow]
    )
