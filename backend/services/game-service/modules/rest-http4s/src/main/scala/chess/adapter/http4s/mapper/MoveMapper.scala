...
/** Maps a [[SubmitMoveRequest]] to a domain [[Move]].
  *
  * Parsing responsibility lives here, not in the route layer. Chess legality is not checked here —
  * that remains the domain's concern.
  */
object MoveMapper:
  ...
  def toDomain(req: SubmitMoveRequest): Either[String, Move] =
    for
      from <- Position
        .fromAlgebraic(req.from)
        .left
        .map(_ => s"Invalid 'from' square: '${req.from}'")
      to <- Position.fromAlgebraic(req.to).left.map(_ => s"Invalid 'to' square: '${req.to}'")
  ...
  /** Parse a promotion piece name.
    *
    * Only the four legal promotion targets are accepted. King and Pawn are rejected with an
    * explicit message.
    */
  private[mapper] def parsePieceType(s: String): Either[String, PieceType] =
    s match
      ...
      case other =>
        Left(s"Invalid promotion piece: '$other'. Must be Queen, Rook, Bishop, or Knight")
