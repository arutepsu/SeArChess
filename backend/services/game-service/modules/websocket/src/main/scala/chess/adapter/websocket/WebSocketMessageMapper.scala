...
/** Maps [[AppEvent]] values to outbound WebSocket JSON messages.
  *
  * ===Transport model===
  * WebSocket is a '''notification channel''', not a full-state transport. Every message signals
  * that something happened and, where useful, carries the minimal fact needed to act on that
  * signal. The canonical game state lives in the REST API; the frontend must re-fetch via `GET
  * /games/{gameId}` after receiving a [[chess.application.event.AppEvent.MoveApplied]] or
  * [[chess.application.event.AppEvent.GameFinished]] notification.
  *
  * ===Message structure===
  ...
  *
  * ===Event categories===
  * All [[AppEvent]] variants are forwarded. See [[chess.application.event.AppEvent]] for the
  * distinction between '''game flow events''' (always relevant to UI) and '''AI monitoring
  * events''' (useful for "AI thinking…" indicators only).
  *
  * ===Field conventions===
  * Consistent with the REST adapter:
  *   - Moves: `{"from": "e2", "to": "e4"}` using algebraic notation; includes `"promotion"` field
  *     only when present.
  *   - Game status: `"status"` string (`"Ongoing"` / `"Checkmate"` / `"Draw"`) plus `"winner"` or
  *     `"drawReason"` as appropriate, matching the REST game response wire convention.
  *   - Colors, lifecycles, modes: `.toString` on the enum case.
  *
  * ===Key field semantics===
  *   - `MoveApplied.playerWhoMoved` — the color that made the move (i.e. whose turn it was *before*
  *     the move). This is NOT the player to move next; re-fetch `GET /games/{gameId}` to obtain the
  *     authoritative `currentPlayer`.
  */
object WebSocketMessageMapper:
  ...
  /** Mutate `obj` in-place with the status-specific fields.
    *
    * Although [[AppEvent.GameFinished]] is only published for terminal states, all four
    * [[GameStatus]] variants are handled for exhaustiveness.
    */
