...
/** Applies http4s CORS middleware to the final composed [[HttpApp]].
  *
  * This is an operational/runtime concern owned by `game-service`. No CORS logic belongs in
  * business route handlers or application services.
  *
  * ===Origin matching===
  *   - `allowedOrigin = "*"` → [[CORS.policy.withAllowOriginAll]] (allow any browser origin;
  *     suitable for open dev environments)
  *   - Any other value → [[CORS.policy.withAllowOriginHeader]] matching the configured origin
  *     string; for example `"http://localhost:3000"` allows only a React/Vite dev server on that
  *     port.
  *
  * Origin matching compares the rendered `scheme://host:port` of the incoming `Origin` header
  * against the configured string.
  *
  * ===Extension path===
  * Multiple allowed origins can be supported later by splitting [[CorsConfig.allowedOrigin]] on
  * commas and checking membership.
  */
object CorsMiddleware:
  ...
  /** Render an [[Origin.Host]] to `"scheme://host"` or `"scheme://host:port"` and compare it to the
