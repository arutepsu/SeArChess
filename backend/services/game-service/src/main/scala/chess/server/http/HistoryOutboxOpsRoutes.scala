...
/** Local/dev read-only visibility for the Game -> History outbox.
  *
  * This is deliberately separate from the public gameplay API. It exposes the SQLite outbox state
  * that already exists; it does not replay, retry, delete, or otherwise mutate delivery rows.
  */
class HistoryOutboxOpsRoutes(outbox: Option[HistoryEventOutbox]):
  ...
  val routes: HttpRoutes[IO] = HttpRoutes.of[IO] {
    ...
    case GET -> Root / "ops" / "history-outbox" / "pending" =>
      withOutbox(
        _.pending(DefaultLimit).map(entries =>
          ujson.Obj(
            "limit" -> DefaultLimit,
          ...
          store.find(id) match
            ...
            case Right(None) =>
              error(
                ...
                s"History outbox entry not found: $id"
              )
  ...
  private def withOutbox(
      read: HistoryEventOutbox => Either[String, ujson.Value]
  ): IO[Response[IO]] =
    outbox match
      case None =>
        error(Status.NotFound, "OUTBOX_NOT_CONFIGURED", "History outbox is not configured")
