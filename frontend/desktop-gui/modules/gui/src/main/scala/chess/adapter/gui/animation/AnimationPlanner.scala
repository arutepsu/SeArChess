...
/** Pure function: derive an [[AnimationPlan]] from before/after board snapshots.
  *
  * The planner MUST use the previous board snapshot to read any captured piece — it is already
  * absent from the post-move board.
  *
  * Castling is excluded: the king travels two squares horizontally, and the rook teleports.
  * Animating castling is deferred to a later phase.
  */
