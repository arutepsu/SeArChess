...
/** Pure mapping layer from [[AnimationState]] to the renderer-facing [[AnimationRenderModel]].
  *
  * This class owns all animation-presentation policy:
  *   - motion-style-driven interpolation of the moving piece position
  *   - explicit phase-based capture choreography: Approach -> Attack -> Attack1 -> Dead -> Fade
  *   - static board suppression (which square the board renderer must skip)
  *
  * It has no ScalaFX dependencies and no mutable state.
  *
  * @param metaRepo
  *   sprite metadata repository for frame-count lookup
  * @param playbackRepo
  *   state playback repository for segment/mode lookup
  */
class AnimationPresentationMapper(
  ...
  /** Map [[state]] to an [[AnimationRenderModel]] for the current frame.
    *
    * @param state
    *   current animation snapshot (progress in [0, 1])
    * @param squareSize
    *   side length of one board square in pixels; defaults to
    *   [[AnimationPresentationMapper.DefaultSquareSize]]
    */
  def map(
  ...
  /** Look up the [[StatePlaybackRepository]] entry for [[piece]] + [[state]], select the active
    * segment via [[PlaybackPlanner]], then compute the frame index via [[FrameSelectionPolicy]].
    *
    * Falls back to a single-segment resolution at frame 0 using the primary asset key when no
    * metadata is found.
    */
  private def resolveSegmented(
    ...
    playbackRepo
      .lookup(piece._1, piece._2, state)
      .map { meta =>
  ...
  /** Resolve a specific segment for a multi-segment visual state.
    *
    * Used for explicit capture choreography where we want: Attack phase -> first attack segment
