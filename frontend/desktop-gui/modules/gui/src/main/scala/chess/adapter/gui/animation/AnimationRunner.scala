...
/** Drives frame-by-frame animation using a JavaFX [[AnimationTimer]].
  *
  * On each timer tick, fires [[onFrame]] with the current [[AnimationState]]. When
  * [[AnimationPlan.durationMs]] has elapsed, fires [[onComplete]] exactly once and stops the timer.
  *
  * Mutable state is intentionally localised here; all callers interact through the two callbacks
  * and [[start]] / [[stop]].
  *
  * @param onFrame
  *   called every animation frame with the current progress snapshot
  * @param onComplete
