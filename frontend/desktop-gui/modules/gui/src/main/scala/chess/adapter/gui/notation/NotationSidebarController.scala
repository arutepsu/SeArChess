...
/** Mutable controller for the notation sidebar.
  ...
  *
  * @param api
  *   the GUI-facing notation API
  * @param stateProvider
  *   callback to obtain the current game state for export actions
  * @param onImportedState
  *   called with the imported [[GameState]] when an import succeeds
  * @param onRefresh
  *   called after every transition with the new sidebar state
  */
class NotationSidebarController(
  ...
  def handle(action: SidebarAction): Unit =
    ...
    val (next, importedStateOpt) =
      NotationSidebarController.transition(sidebarState, action, api, currentGame)
    sidebarState = next
...
object NotationSidebarController:
  ...
  /** Pure transition from one [[NotationSidebarState]] to the next.
    ...
    *   the GUI-facing notation API
    * @param currentGame
    *   the current game state, provided by the caller for export
    * @return
    *   (next sidebar state, Some(importedGameState) if an import succeeded)
    */
  def transition(
  ...
  /** Map a [[NotationActionId]] to the appropriate [[GuiNotationApi]] call and handler.
    *
    * This is the single place where action identifiers are resolved to API methods. Adding a new
    * notation action requires only extending this match and registering a descriptor in
    * [[NotationActionDescriptor.defaults]].
    */
  ...
  /** Map an import outcome to the next state and an optional imported [[GameState]].
    *
    * On success: update feedback, preserve warnings, signal the imported state. On failure: update
    * feedback, clear warnings and output, do not signal.
    */
  ...
  /** Map an export outcome to the next state.
    *
    * On success: populate output text, clear feedback. On failure: populate feedback with readable
    * message, clear output text.
    */
