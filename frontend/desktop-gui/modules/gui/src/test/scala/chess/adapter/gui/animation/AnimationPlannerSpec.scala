...
class AnimationPlannerSpec extends AnyFlatSpec with Matchers:
  ...
  it should "record the correct moving piece for a normal move" in {
    val board = Board.empty.place(e2, whitePawn)
    val plan = AnimationPlanner.plan(board, Move(e2, e4)).getOrElse(fail("expected Some plan"))
  ...
  it should "record the captured piece from the previous board snapshot" in {
    val board = Board.empty.place(e2, whitePawn).place(d5, blackPawn)
    val plan = AnimationPlanner.plan(board, Move(e2, d5)).getOrElse(fail("expected Some plan"))
  ...
  it should "return a plan for a king moving one square (not castling)" in {
    ...
    plan shouldBe defined
    plan.getOrElse(fail("expected Some plan")).movingPiece shouldBe (Color.White, PieceType.King)
  }
  ...
  it should "use a longer duration for a capture than for a normal move" in {
    val normalPlan = AnimationPlanner
      .plan(Board.empty.place(e2, whitePawn), Move(e2, e4))
      .getOrElse(fail("expected Some plan"))
    val capturePlan =
      AnimationPlanner
        .plan(Board.empty.place(e2, whitePawn).place(d5, blackPawn), Move(e2, d5))
        .getOrElse(fail("expected Some plan"))
