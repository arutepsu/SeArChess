...
class MotionInterpolatorSpec extends AnyFlatSpec with Matchers:
  ...
  it should "be slower than Linear near t=0 (ease-in)" in {
    val (xSmooth, _) =
      MotionInterpolator.interpolate(MotionStyle.Smooth, fromX, fromY, toX, toY, 0.1)
    val (xLinear, _) =
      MotionInterpolator.interpolate(MotionStyle.Linear, fromX, fromY, toX, toY, 0.1)
  ...
  it should "be faster than Linear near t=0.5 then slow down (ease-out)" in {
    ...
    val (xSmooth, _) =
      MotionInterpolator.interpolate(MotionStyle.Smooth, fromX, fromY, toX, toY, 0.9)
    val (xLinear, _) =
      MotionInterpolator.interpolate(MotionStyle.Linear, fromX, fromY, toX, toY, 0.9)
    xSmooth should be > xLinear
  }
  ...
  it should "be slower than Smooth near t=0 (heavier ease-in)" in {
    ...
    val (xSmooth, _) =
      MotionInterpolator.interpolate(MotionStyle.Smooth, fromX, fromY, toX, toY, 0.2)
  ...
  it should "lift the piece above the straight-line path at t=0.5" in {
    ...
    val (_, yArc) =
      MotionInterpolator.interpolate(MotionStyle.Arc(0.5), fromX, fromY, toX, toY, 0.5)
    val (_, yLinear) =
      MotionInterpolator.interpolate(MotionStyle.Linear, fromX, fromY, toX, toY, 0.5)
    yArc should be < yLinear
  }

  it should "produce zero lift when heightFraction is 0" in {
    val (_, yArc) =
      MotionInterpolator.interpolate(MotionStyle.Arc(0.0), fromX, fromY, toX, toY, 0.5)
    val (_, ySmooth) =
      MotionInterpolator.interpolate(MotionStyle.Smooth, fromX, fromY, toX, toY, 0.5)
  ...
  "MotionInterpolator.interpolate (AttackLunge)" should "place piece at source at t=0" in {
    val (x, y) =
      MotionInterpolator.interpolate(MotionStyle.AttackLunge(0.15), 0.0, 0.0, 400.0, 0.0, 0.0)
    x shouldBe (0.0 +- eps); y shouldBe (0.0 +- eps)
  }

  it should "place piece at destination at t=1" in {
    val (x, y) =
      MotionInterpolator.interpolate(MotionStyle.AttackLunge(0.15), 0.0, 0.0, 400.0, 0.0, 1.0)
  ...
  it should "overshoot past the destination at t=impactT (horizontal)" in {
    ...
    val impT = 0.65
    val (x, _) =
      MotionInterpolator.interpolate(MotionStyle.AttackLunge(0.15), 0.0, 0.0, 400.0, 0.0, impT)
    x should be > 400.0
  }
  ...
  it should "produce no overshoot when overshootFraction is 0" in {
    ...
    val impT = 0.65
    val (x, y) =
      MotionInterpolator.interpolate(MotionStyle.AttackLunge(0.0), 0.0, 0.0, 400.0, 0.0, impT)
    x shouldBe (400.0 +- eps); y shouldBe (0.0 +- eps)
  }
  ...
  it should "overshoot vertically for a vertical capture" in {
    ...
    val impT = 0.65
    val (x, y) =
      MotionInterpolator.interpolate(MotionStyle.AttackLunge(0.15), 0.0, 0.0, 0.0, 400.0, impT)
  ...
  it should "overshoot diagonally for a diagonal capture" in {
    ...
    val impT = 0.65
    val (x, y) =
      MotionInterpolator.interpolate(MotionStyle.AttackLunge(0.15), 0.0, 0.0, 300.0, 400.0, impT)
  ...
  it should "return destination immediately when source equals destination" in {
    val (x, y) =
      MotionInterpolator.interpolate(MotionStyle.AttackLunge(0.15), 200.0, 150.0, 200.0, 150.0, 0.5)
    x shouldBe 200.0; y shouldBe 150.0
  }
