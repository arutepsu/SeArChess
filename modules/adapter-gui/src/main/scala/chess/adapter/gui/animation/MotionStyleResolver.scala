...
object MotionStyleResolver:
   ...
   *
   *  @param pieceType the type of the moving piece
   *  @param isCapture `true` when the move is a capture; the attacker uses
   *                   [[MotionStyle.AttackLunge]] regardless of piece type
   */
  def resolve(pieceType: PieceType, isCapture: Boolean = false): MotionStyle =
    if isCapture then MotionStyle.AttackLunge(0.15)
    else pieceType match
