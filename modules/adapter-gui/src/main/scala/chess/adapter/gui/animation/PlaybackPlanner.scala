 ...
 *
 *  Maps a global normalized progress in `[0, 1]` to the active segment asset
 *  key and a local normalized progress within that segment, ready for
 *  [[chess.adapter.gui.assets.FrameSelectionPolicy.select]].
 *
 *  === Segment distribution ===
 *  Progress is divided evenly across all segments.  The active segment index is
 *  `floor(progress × segmentCount)` clamped to `[0, segmentCount − 1]`; no
 *  branching is needed beyond the mode normalisation.
 *
 *  === PlaybackMode ===
 *  Applied before distribution:
 *  - `Clamp`: global progress is clamped to `[0, 1]`.
 *  - `Loop`: global progress is reduced to its fractional part
 *    (`p − floor(p)`), repeating the sequence indefinitely.
 */
object PlaybackPlanner:

  /** Result of segment selection for one animation frame.
   *
   *  @param segmentAssetKey asset key of the sprite-sheet segment that is active
   *                         at the given progress (e.g. `"classic/white_pawn_attack1"`)
   *  @param localProgress   normalised progress within this segment, in `[0, 1]`;
   *                         ready for [[chess.adapter.gui.assets.FrameSelectionPolicy.select]]
   */
   ...
   *
   *  @param metadata  playback descriptor for the logical state
   *  @param progress  overall animation progress; may be outside `[0, 1]`
   *  @return          the active segment asset key and local progress in `[0, 1]`
   */
