...
/** Whether a notation action reads from the input text field or serialises
 ...
 *  [[SidebarAction.NotationActionRequested]] with the matching [[NotationActionId]].
 *  The controller holds the mapping from id to [[GuiNotationApi]] method.
 *
 *  @param id    typed action identifier consumed by the controller
 *  @param label human-readable button label shown in the sidebar
 *  @param kind  whether this is an import or export action (used for layout grouping)
 */
...
object NotationActionDescriptor:
   ...
   *
   *  Passed to [[NotationSidebar]] at construction; callers may substitute a
   *  filtered or reordered set (e.g. to disable PGN while it is unavailable,
   *  or to add future formats without changing the widget).
   */
