...
/** Tests focused on [[SessionService.getSessionByGameId]], the method added
 *  for the Phase 7 REST adapter.
 *
 *  Other [[SessionService]] methods are exercised transitively by the REST
 *  mapper and route integration paths; dedicated coverage for them can be
 *  added in Phase 8.
 */
class SessionServiceSpec extends AnyFlatSpec with Matchers with EitherValues:

  private def freshService =
    val repo = InMemorySessionRepository()
    (SessionService(repo, _ => ()), repo)

  private def createSession(service: SessionService): (GameId, GameSession) =
  ...
  "SessionService.getSessionByGameId" should "retrieve the session for a known GameId" in {
