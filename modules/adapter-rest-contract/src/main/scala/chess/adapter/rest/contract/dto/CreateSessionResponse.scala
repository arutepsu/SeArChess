...
/** Canonical response body for creating a playable game via POST /sessions.
  *
  * Bundles the new session metadata with the initial game state so the client has everything it
  * needs to start interacting without a second round-trip.
  *
  * @param session
  *   session metadata
  * @param game
  *   full initial game snapshot; wire field name remains "game"
  */
