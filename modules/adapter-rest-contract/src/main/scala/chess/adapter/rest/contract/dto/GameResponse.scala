...
/** Canonical wire representation of authoritative game state.
  *
  * @param gameId
  *   opaque UUID string identifying the game record
  * @param currentPlayer
  *   "White" or "Black"
  * @param status
  *   "Ongoing", "Checkmate", "Draw", or "Resigned"
  * @param inCheck
  *   true when status is Ongoing and the current player is in check
  * @param winner
  *   present when status is "Checkmate"; "White" or "Black"
  * @param drawReason
  *   present when status is "Draw"; e.g. "Stalemate"
  ...
  * @param moveHistory
  *   ordered list of moves played so far
  * @param lastMove
  *   the most recent move, if any
  * @param promotionPending
  *   true when the position requires a promotion choice before play can continue; false in all
  *   current REST v1 flows
  * @param legalTargetsByFrom
  *   legal target squares for each current-player source square; keys are source squares with at
