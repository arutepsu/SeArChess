...
/** Request body for POST /games/{gameId}/moves.
  ...
  * @param promotion
  *   promotion piece type when a pawn reaches the back rank; one of "Queen" | "Rook" | "Bishop" |
  *   "Knight"
  * @param controller
  *   the controller submitting the move; defaults to "HumanLocal" when absent. The Game Service
  *   maps this wire value to its internal command model.
  */
...
object SubmitMoveRequest:
  /** Parse from a JSON string.
    *
    * `from` and `to` are required; returns Left if either is absent or the JSON is malformed.
    */
