...
object RemoteAiJson:
  ...
  def requestToJson(request: RemoteAiMoveSuggestionRequest): String =
    ujson.write(
      ujson.Obj(
        ...
        "engine" -> ujson.Obj.from(
          request.engine.engineId.map(id => "engineId" -> (ujson.Str(id): Value))
        ),
        "limits" -> ujson.Obj(
  ...
  def requestFromJson(text: String): Either[String, RemoteAiMoveSuggestionRequest] =
    for
      ...
      moves <- movesJson.value.toList.zipWithIndex
        .foldLeft[Either[String, List[RemoteAiMoveDto]]](Right(Nil)) {
      ...
      metadataObj = optionalObj(json, "metadata").getOrElse(
        ujson.Obj("mode" -> ujson.Str("Unknown"))
      )
  ...
  def responseFromJson(text: String): Either[String, RemoteAiMoveSuggestionResponse] =
    ...
    yield RemoteAiMoveSuggestionResponse(
      requestId,
      move,
      engineId,
      engineVersion,
  ...
  private def requiredArr(
      ...
      field: String,
      owner: String
  ): Either[String, ujson.Arr] =
    json.obj
      .get(field)
      .collect { case arr: ujson.Arr => arr }
