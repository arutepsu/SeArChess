 ...
 *
 *  - [[NotConfigured]]: no AI service is wired into this deployment.
 *    The operation cannot be attempted at all.
 *  - [[NotAITurn]]: the current player's side is not AI-controlled.
 *    The AI turn guard rejected the request; this is a policy/state error,
 *    not an infrastructure error.
 *  - [[SessionLookupFailed]]: the session could not be loaded from the
 *    repository.  Carries the underlying [[SessionError]] so callers can
 ...
 *  [[SessionLookupFailed]] and [[GameStateLookupFailed]] currently embed
 *  [[SessionError]] and [[RepositoryError]] from lower application layers.
 *  This leaks internal boundary types into the public service contract.
 *  A later cleanup should flatten these into self-contained cases at the
 *  service-boundary level (e.g. `SessionNotFound(id)`, `GameStateNotFound(id)`,
 *  `StorageFailure(msg)`) so that callers do not depend on the internal
 *  error hierarchy.
 */
enum AITurnError:
  /** No AI service is wired into this deployment; [[chess.application.GameServiceApi.triggerAIMove]]
   *  is not available. */
  case NotConfigured
