 ...
 *  Implementations must ensure that `getState` and `updateState` are safe to
 *  call from concurrent threads.  Observer callbacks should be invoked outside
 *  any lock so that:
 *  (a) slow observers do not starve state readers, and
 *  (b) observers that call `getState` or `updateState` themselves do not
...
trait GameStateObservable:
   ...
   *
   *  Observers are called with a snapshot of the new state after any internal
   *  lock is released.
   */
