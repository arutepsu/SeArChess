 ...
 *
 *  The canonical entry point for all chess rule queries and state transitions.
 *  Application and adapter layers must use this object rather than composing
 *  raw rule objects directly.
 */
object GameStateRules:

  /** All legal target squares that the current-player piece at `from` can
   *  move to, including promotion targets.  Returns an empty set when there is
   ...
   *  Returns an empty set when there is no current-player piece at `from`.
   */
   ...
   *
   *  Handles all pure state derivation: board update, castling rights,
   *  en passant lifecycle, halfmove clock, fullmove number, and status
   *  re-evaluation.  Returns `Left(MissingPromotionChoice)` when a pawn
   *  reaches the last rank without a promotion piece specified.
   *
    ...
    MoveApplier.applyMove(state.board, move, state.castlingRights, state.enPassantState)
      .flatMap {
        case MoveResult.Applied(newBoard) =>
          ...
          val nextFullmove  = if state.currentPlayer == Color.Black then state.fullmoveNumber + 1
