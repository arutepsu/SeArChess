...
import chess.domain.rules.validation.{
  CastlingValidator,
  CheckValidator,
  EnPassantValidator,
  MoveValidator
...
object MoveApplier:
  ...
  /** Apply a move to a board, enforcing legal movement rules and king safety.
    ...
    *   - Right(PromotionRequired(board, square, color)) — pawn reached the last rank but
    *     `move.promotion` is None; caller must supply a promotion piece
    *
    * When `move.promotion` is defined and a pawn reaches the last rank the promotion is applied
    * inline and Applied is returned directly.
    *
    * Fails with:
    ...
    *   1. empty source 2. castling (King + castling geometry) 3. en passant (Pawn + active en
    *      passant state + matching target) 4. normal move
    */
  def applyMove(
      board: Board,
      move: Move,
    ...
    board.pieceAt(move.from) match
      ...
      case Some(capturingPiece)
          if EnPassantValidator.isEnPassantMove(board, move, enPassantState) =>
        for
