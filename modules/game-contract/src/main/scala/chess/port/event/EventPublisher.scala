...
/** Outbound port for application-layer event publication.
  *
  * Services call [[publish]] after a meaningful state transition completes. The port is
  * fire-and-forget: it returns [[Unit]] and must not throw; any implementation failure should be
  * handled internally (e.g. logged and silently dropped) so that event publication never breaks a
  * service call.
  *
...
trait EventPublisher:
  ...
  /** Publish a single [[AppEvent]].
    *
    * Must not throw. The caller does not know or care what happens next; publication is a
