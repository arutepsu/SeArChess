 ...
 *
 *  Defines the authoritative write surface for session-aware game creation and
 *  move submission.  Every operation behind this interface:
 *
 *  1. validates a command against domain rules and session policy
 *  2. persists session metadata and game state as one logical unit via
 *     [[chess.application.port.repository.SessionGameStore]]
 *  3. emits post-persistence [[chess.application.event.AppEvent]]s
 *
 *  === Future extraction boundary ===
 *  This interface marks the first cleanly extractable service boundary.
 *  A future game-session command service would implement this trait and own:
 *  - the write model for session metadata and game state (see [[chess.application.port.repository.SessionGameStore]])
 ...
 *
 *  Adapters that depend on this interface — rather than on [[SessionGameService]]
 *  directly — can be re-pointed to a remote implementation without changing their
 *  own logic.
 *
 *  === What this interface does NOT cover ===
 *  - read / query access to session or game state (use [[SessionService]] or a
 *    dedicated query service)
 *  - session-only lifecycle writes that do not involve a [[GameState]] update
 *    (e.g. [[SessionService.preparePromotion]], [[SessionService.updateLifecycle]])
...
trait GameSessionCommands:
   ...
   *
   *  On success: the session and the initial [[GameState]] are saved as one write
   *  via [[chess.application.port.repository.SessionGameStore]], then
   *  [[chess.application.event.AppEvent.SessionCreated]] is published.
   *
   *  @return the fresh game state and its new session, or a [[SessionError]]
   */
  def newGame(
    mode:            SessionMode,
   ...
   *  [[chess.application.event.AppEvent.SessionLifecycleChanged]] if applicable.
   ...
   *
   *  Sets [[chess.domain.model.GameStatus.Resigned]] on the game state and
   *  transitions the session to [[chess.application.session.model.SessionLifecycle.Finished]].
   *  Both writes are committed atomically via
   *  [[chess.application.port.repository.SessionGameStore]].
   *
   ...
   *  @param session       current live session (caller must have loaded it)
   *  @param state         current game state (caller must have loaded it)
   *  @param resigningSide the [[chess.domain.model.Color]] that is conceding
   *  @param now           wall-clock instant for lifecycle timestamps
   *  @return updated (GameState, GameSession) or a [[SessionError]]
   */
  def resignGame(
    session:      GameSession,
    state:        GameState,
