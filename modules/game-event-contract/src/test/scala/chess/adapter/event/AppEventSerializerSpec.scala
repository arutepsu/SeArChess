...
/** Tests for [[AppEventSerializer]].
  *
  * Covers: wire type names (stability), required field presence, payload correctness for all five
  * boundary events, promotion encoding, controller string encoding, non-boundary event passthrough,
  * and terminal result encoding for Checkmate and Draw.
  */
class AppEventSerializerSpec extends AnyFlatSpec with Matchers with EitherValues with OptionValues:
  ...
  it should "encode AI controller with engine id as 'AI:<engineId>'" in {
    val event = AppEvent.SessionCreated(
      ...
      gid,
      SessionMode.AIVsAI,
      SideController.AI(Some("stockfish-15")),
      SideController.AI(Some("random"))
    )
    val obj = parse(AppEventSerializer.serialize(event).value)
  ...
  it should "encode promotion piece type when move promotes a pawn" in {
    val event =
      AppEvent.MoveApplied(sid, gid, Move(pos("e7"), pos("e8"), Some(PieceType.Queen)), Color.White)
    val obj = parse(AppEventSerializer.serialize(event).value)
