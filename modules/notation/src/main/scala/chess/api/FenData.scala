 ...
 *
 *  Produced by the FEN parser and embedded in [[ParsedNotation.ParsedFen]].
 *  Carries the six FEN fields in a typed form so that the importer never
 *  needs to re-parse the raw string.
 *
 *  The nested types mirror the six FEN fields without coupling the shared
 *  notation contract to implementation-specific parser types in
 *  `chess.notation.fen`.
 *
 *  `ranks` is ordered from FEN rank 8 (index 0) to FEN rank 1 (index 7),
 *  matching the left-to-right field order in the FEN string.
 */
