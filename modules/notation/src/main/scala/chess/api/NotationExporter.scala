 ...
 *
 *  One implementation per supported export format.
 *  Implementations are permitted to reject unsupported [[NotationFormat]] values
 *  via [[ExportFailure.UnsupportedExportFormat]].
 */
trait NotationExporter[A]:
  /** Serialise `data` to the notation text described by `format`.
   *
   *  @param data   the domain-side value to serialise
   *  @param format the target notation format
   *  @return [[Right]] containing the structured [[ExportResult]] on success,
   *          [[Left]]  containing a structured [[NotationFailure]] on any failure
   */
