 ...
 *
 *  All failures carry at least a `message`.  Sub-hierarchies refine the
 *  category so callers can branch precisely without matching raw strings.
 *
 *  Hierarchy:
 ...
 *
 *  Parsers return `Either[ParseFailure, ParsedNotation]` so callers that only
 *  invoke the parser see a narrower error type.
 */
...
/** A failure that occurs when a syntactically valid representation contains
 *  semantically invalid data (e.g. an illegal position, an out-of-range value).
 */
sealed trait ValidationFailure extends NotationFailure

object ValidationFailure:
  /** A field's value is not in the set of legal values. */
  final case class InvalidValue(field: String, value: String, message: String) extends ValidationFailure
  ...
  final case class OutOfRange(field: String, value: String, message: String) extends ValidationFailure
...
object ImportFailure:
   ...
   *
   *  @param parsedKind  the kind of [[ParsedNotation]] that was presented
   *  @param target      the [[ImportTarget]] that was requested
   *  @param message     human-readable explanation of the incompatibility
   */
...
/** A failure indicating that the input uses a dialect or version the parser
 *  does not support.
 */
