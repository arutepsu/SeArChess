...
/** Parser-local color representation.
  *
  * Corresponds to the FEN active-color tokens 'w' and 'b'. Not a domain type — the importer
  * converts this to the domain Color.
  */
enum FenColor:
  case White, Black
...
/** Parser-local piece-type representation.
  *
  * Corresponds to uppercase/lowercase FEN piece characters. Not a domain type — the importer
  * converts this to the domain PieceType.
  */
enum FenPieceSymbol:
...
/** Complete structured representation of a parsed FEN string.
  *
  * This is a parser-local model internal to the `chess.notation.fen` package. It is NOT part of the
  * shared notation API and must not be used as a contract.
  *
  * `ranks` is ordered from FEN rank 8 (index 0) to FEN rank 1 (index 7), matching the left-to-right
  * field order in the FEN string. Any reordering needed by the domain board model is the importer's
  * responsibility.
  */
