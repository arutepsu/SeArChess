...
/** Pure FEN serializer.
  ...
  *
  * Implements [[NotationExporter]] for [[GameState]] and rejects all [[NotationFormat]] values
  * other than [[NotationFormat.FEN]].
  *
  * Field order:
  * {{{
  ...
  * }}}
  *
  * Clock fields: `halfmoveClock` and `fullmoveNumber` are read directly from [[GameState]]. The FEN
  * import path writes them, so round-tripping a position through FEN will preserve the correct
  * values.
  */
object FenSerializer extends NotationExporter[GameState]:
      ...
      case other =>
        Left(
          ExportFailure.UnsupportedExportFormat(
            other,
            s"FenSerializer only supports FEN; got $other"
          )
        )
  ...
  /** Serialize piece placement, rank 8 → rank 1, file a → h.
    *
    * Consecutive empty squares are compressed to a digit (1–8). Ranks are separated by `/`.
    */
  private[fen] def serializePiecePlacement(state: GameState): String =
    (7 to 0 by -1)
      .map { rank =>
        ...
        for file <- 0 to 7 do
          state.board.pieceAt(
            chess.domain.model.Position
              .from(file, rank)
              .getOrElse(throw AssertionError(s"Invalid board position: file=$file rank=$rank"))
          ) match
            case Some(piece) =>
  ...
  /** Serialize en passant target.
    *
    * Uses [[EnPassantState.targetSquare]], the square a capturing pawn moves into (not the square
    * of the captured pawn). Returns `-` if absent.
    */
