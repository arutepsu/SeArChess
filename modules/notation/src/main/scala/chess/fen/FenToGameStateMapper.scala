...
/** Maps a semantically validated [[FenData]] into a domain [[GameState]].
  *
  * All methods are pure: no side effects, no board rule logic, no domain validation. Semantic
  * validation must happen in [[FenSemanticValidator]] before this mapper is called.
  *
  * Mapping policy:
  *   - `board`, `currentPlayer`, `castlingRights`, `enPassantState` — derived from FenData
  *   - `status` — computed by [[GameStatusEvaluator]] to reflect the real game state (check /
  *     checkmate / stalemate / ongoing)
...
object FenToGameStateMapper:
  ...
  /** Build a domain [[Board]] from the FEN rank vectors.
    *
    * FEN rank ordering: `ranks(0)` = rank 8 (0-based rank 7), `ranks(7)` = rank 1 (0-based rank 0).
    * Within each rank, `squares(fileIdx)` = file `fileIdx`.
    */
      ...
      rankSquares.zipWithIndex.foldLeft(board) { case (b, (square, fileIdx)) =>
        square match
          ...
          case FenData.Square.Occupied(color, symbol) =>
            Position
              .from(fileIdx, boardRank)
              .toOption
              .fold(b)(pos => b.place(pos, Piece(mapColor(color), mapPieceSymbol(symbol))))
      }
    }
  ...
  /** Map the FEN en passant field to a domain [[EnPassantState]].
    *
    * The FEN target square is the empty square the capturing pawn moves INTO. The capturable pawn
    * occupies the adjacent square in the opposite direction:
    *   - When White is active (White captures): Black's pawn is one rank below the target (rank - 1
    *     in 0-based).
    *   - When Black is active (Black captures): White's pawn is one rank above the target (rank + 1
    *     in 0-based).
    */
  private def mapEnPassant(
    ...
    ep match
      ...
      case FenData.EnPassantTarget.Square(file, rank) =>
        val capturablePawnRank =
          if activeColor == FenData.ActiveColor.White then rank - 1 else rank + 1
        val pawnColor =
          if activeColor == FenData.ActiveColor.White then Color.Black else Color.White
        for
