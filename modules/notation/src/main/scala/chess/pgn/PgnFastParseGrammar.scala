...
private[pgn] object PgnFastParseGrammar extends PgnGrammar:
  ...
  /** SAN token kept intentionally permissive.
    *
    * Legality and deeper SAN interpretation happen later in [[SanResolver]]. This parser just
    * extracts token boundaries for the PGN core.
    */
  private def sanToken[$: P]: P[String] =
    P(CharsWhile(ch => !ch.isWhitespace && ch != '[' && ch != ']', min = 1).!)
      .filter(!ResultTokens.contains(_))
      .opaque("SAN token")
