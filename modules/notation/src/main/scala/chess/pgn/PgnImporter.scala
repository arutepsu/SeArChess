...
/** Maps a [[ParsedNotation.ParsedPgn]] to a [[ImportResult.GameImportResult]] by replaying the SAN
  * mainline tokens from the standard starting position.
  *
  * Target and kind routing:
  *   - [[ImportTarget.PositionTarget]] → [[ImportFailure.IncompatibleTarget]] (PGN represents a
  *     game, not a bare position)
  *   - Any non-PGN [[ParsedNotation]] → [[ImportFailure.IncompatibleTarget]]
  *   - PGN with SetUp/FEN headers → [[CompatibilityFailure.UnsupportedDialect]] (non-standard
  *     starting positions are not yet supported)
  *
...
private[pgn] object PgnImporter extends NotationImporter[GameState]:
  ...
  private def importGame(
    ...
    if pgn.data.headers.get("SetUp").contains("1") && pgn.data.headers.contains("FEN") then
      return Left(
        CompatibilityFailure.UnsupportedDialect(
          "SetUp/FEN",
