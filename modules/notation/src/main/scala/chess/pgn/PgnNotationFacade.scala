 ...
 *  - `exportWithHeaders` — assembles a complete PGN document from caller-supplied
 *                          headers and the move body derived from [[PgnExporter]];
 *                          intended for History / archive materialisation where
...
object PgnNotationFacade extends NotationFacade[GameState]:
  ...
  /** Assemble a complete PGN document with caller-supplied headers and the
   *  move body derived from [[data]].
   *
   *  Headers are emitted in the order provided — callers must supply the
   *  Seven Tag Roster in the standard PGN order (Event, Site, Date, Round,
   *  White, Black, Result) if compliance is required.  A blank line separates
   *  the header block from the movetext, as required by the PGN standard.
   *
   *  An empty [[headers]] sequence produces a header-less document identical
   *  to [[executeExport]] with [[NotationFormat.PGN]].
   *
   *  @param data    the game state whose move history is serialised as movetext
   *  @param headers ordered tag pairs (name → value) to emit before the movetext
   *  @return [[ExportResult]] whose `text` is a well-formed PGN document, or a
   *          [[NotationFailure]] if move serialisation fails
   */
