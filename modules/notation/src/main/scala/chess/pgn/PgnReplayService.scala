 ...
 *
 *  Each token is resolved to a [[chess.domain.model.Move]] via [[SanResolver]],
 *  then applied to the evolving [[GameState]] via [[GameStateRules.applyMove]].
 *  Replay stops and returns a failure on the first token that cannot be resolved
 *  or applied.
 *
...
private[pgn] object PgnReplayService:
   ...
   *
   *  @param initialState the starting [[GameState]] — callers choose this explicitly
   *  @param tokens       SAN half-move tokens extracted from a PGN mainline
   *  @return the final [[GameState]] after all moves are applied, or a
   *          [[NotationFailure]] describing the first error
   */
  def replayFrom(initialState: GameState, tokens: Vector[String]): Either[NotationFailure, GameState] =
