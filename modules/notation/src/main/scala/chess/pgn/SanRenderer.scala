...
/** Renders a single [[Move]] to its SAN string given the board state immediately before the move is
  * played.
  *
...
private[pgn] object SanRenderer:
  ...
  /** Render `move` to its SAN token in the context of `stateBefore`.
    *
    * @param stateBefore
    *   the [[GameState]] immediately before `move` is applied
    * @param move
    *   the move to render
    * @return
    *   the SAN string on success, or a [[NotationFailure]] when the pre-move state is inconsistent
    *   with the move (e.g. no piece at `move.from`, or `applyMove` fails unexpectedly)
    */
  ...
  private def buildBase(state: GameState, move: Move, piece: Piece): String =
    if isCastling(piece, move) then if move.to.file > move.from.file then "O-O" else "O-O-O"
    else
