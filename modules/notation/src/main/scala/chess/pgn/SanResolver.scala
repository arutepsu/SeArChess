...
/** Resolves a single SAN token to a legal [[Move]] in the given game state.
  *
  * SAN (Standard Algebraic Notation) encodes only the destination square and enough disambiguation
  * to identify a unique moving piece, so the current board position is required to find the `from`
  * square.
  *
  * Supported tokens:
  *   - pawn pushes: `e4`
...
private[pgn] object SanResolver:
  ...
  def resolve(state: GameState, san: String): Either[NotationFailure, Move] =
    ...
    if stripped == "O-O-O" || stripped == "0-0-0" then resolveQueenSideCastle(state)
    else if stripped == "O-O" || stripped == "0-0" then resolveKingSideCastle(state)
    else if stripped.contains("=") then resolvePromotion(state, stripped)
    else
            ...
            case None =>
              Left(
                ValidationFailure.InvalidValue(
                  "san",
                  san,
                  s"Could not parse destination square in: '$san'"
                )
              )
            case Some(dest) =>
      ...
      case None =>
        return Left(
          ValidationFailure.InvalidValue(
            "san",
            san,
            s"Unknown promotion piece '$promoPart' in: '$san'"
          )
        )
      case Some(p) => p
      ...
      case None =>
        Left(
          ValidationFailure.InvalidValue(
            "san",
            san,
            s"Cannot parse move part of promotion token: '$san'"
          )
        )
          ...
          case None =>
            Left(
              ValidationFailure.InvalidValue(
                "san",
                san,
                s"No destination square in promotion token: '$san'"
              )
            )
          case Some(dest) =>
            findUniqueMove(
  ...
  /** Returns (pieceType, destSquare, fileHint, rankHint, isCapture).
    *
    * `fileHint` / `rankHint` disambiguate when multiple pieces of the same type can reach the
    * destination. Both are `None` when there is no ambiguity in the source text.
    */
  ...
  private def validateLegal(
      state: GameState,
      move: Move,
      san: String
  ): Either[NotationFailure, Unit] =
