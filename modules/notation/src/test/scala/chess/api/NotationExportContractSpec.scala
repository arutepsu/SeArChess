...
class NotationExportContractSpec extends AnyFlatSpec with Matchers with EitherValues:
  ...
  "ExportFailure" should "be a NotationFailure" in {
    val f: NotationFailure = ExportFailure.UnsupportedExportFormat(NotationFormat.PGN, "not implemented")
    f.message should include("not implemented")
  }

  it should "distinguish UnsupportedExportFormat from SerializationError" in {
    val unsupported: ExportFailure = ExportFailure.UnsupportedExportFormat(NotationFormat.JSON, "no JSON exporter")
    val serialErr:   ExportFailure = ExportFailure.SerializationError("halfmoveClock", "value out of range")
  ...
  it should "carry the target format in UnsupportedExportFormat" in {
    val f = ExportFailure.UnsupportedExportFormat(NotationFormat.FEN, "FEN export not yet available")
  ...
  private object StubFenExporter extends NotationExporter[String]:
    def exportNotation(data: String, format: NotationFormat): Either[NotationFailure, ExportResult] =
      format match
        case NotationFormat.FEN =>
          Right(ExportResult(text = s"FEN:$data", format = NotationFormat.FEN))
        case other =>
          Left(ExportFailure.UnsupportedExportFormat(other, s"$other not supported by StubFenExporter"))
  ...
  it should "carry the rejected format in the UnsupportedExportFormat failure" in {
    ...
    result.left.value.asInstanceOf[ExportFailure.UnsupportedExportFormat].format shouldBe NotationFormat.JSON
  }
  ...
  private object StubImportOnlyFacade extends NotationFacade[String]:
    ...
    def executeImport(parsed: ParsedNotation, target: ImportTarget): Either[NotationFailure, ImportResult[String]] =
  ...
  it should "include the requested format in the failure" in {
    ...
    result.left.value.asInstanceOf[ExportFailure.UnsupportedExportFormat].format shouldBe NotationFormat.FEN
  }
  ...
  private object StubExportAwareFacade extends NotationFacade[String]:
    ...
    def executeImport(parsed: ParsedNotation, target: ImportTarget): Either[NotationFailure, ImportResult[String]] =
      Left(ImportFailure.MappingError("stub"))
    override def executeExport(data: String, format: NotationFormat): Either[NotationFailure, ExportResult] =
