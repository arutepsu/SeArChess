...
final class FenFastParseGrammarSpec extends AnyFlatSpec with Matchers with EitherValues:

  private def parse(input: String): Either[ParseFailure, FenRecord] =
    FenFastParseGrammar.parseRecord(input)
  ...
  "FenFastParseGrammar.parseRecord" should "return UnexpectedEndOfInput for empty input" in {
    parse("") shouldBe Left(
  ...
  it should "reject FEN with double spaces because whitespace is strict" in {
  ...
  it should "reject duplicate castling symbols" in {
    val err = syntaxError("8/8/8/8/8/8/8/8 w KK - 0 1")
    err.message should include("contains duplicate symbols")
  ...
  it should "reject trailing whitespace because full input must be consumed" in {
    parse("8/8/8/8/8/8/8/8 w - - 0 1 ").left.value shouldBe a[ParseFailure.SyntaxError]
  }
