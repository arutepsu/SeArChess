 ...
 *  @param frameIndex       zero-based sprite-sheet frame to display; computed by
 *                          [[chess.adapter.gui.assets.SequencePlaybackPolicy]] in
 *                          [[AnimationPresentationMapper]]; defaults to 0 for static pieces
 *  @param segmentAssetKey  asset key of the specific sprite-sheet segment to display;
 *                          overrides the default key derived from [[PieceVisualId]] when
 *                          set (used for multi-segment states such as Attack); [[None]]
 *                          means use the primary key for the state
 *  @param flipX            when `true` the sprite should be rendered mirrored horizontally;
 *                          used to face the piece in the direction of travel
 *  @param visualState      the actual [[VisualState]] being rendered; used by the scene to build
 *                          the correct [[chess.adapter.gui.assets.PieceVisualId]]
 *  @param scale            uniform scale factor applied to the sprite; `1.0` = normal size;
 *                          values above `1.0` produce a momentary pop effect (e.g. hit reaction)
 */
