 ...
 */
class PieceNodeFactory(loader: SpriteSheetLoader):
   ...
   */
  def content(id: PieceVisualId, squareSize: Double, frameIndex: Int = 0): Node =
    val descriptor = VisualResolver.resolve(id)
    val maybeImage = for
      meta  <- SpriteMetadataRepository.lookup(descriptor.assetKey)
      image <- loader.loadFrame(descriptor.assetKey, frameIndex, meta)
    yield
      ...
      new ImageView(new Image(image)):
        ...
        fitHeight       = dispH
        preserveRatio   = false
    maybeImage.getOrElse(fallbackGlyph(descriptor, id, squareSize))
  ...
  def positioned(
    ...
    val pieceContent = content(id, squareSize, frameIndex)
