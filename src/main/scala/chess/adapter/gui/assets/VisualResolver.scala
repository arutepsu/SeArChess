 ...
 *  consume the descriptor returned here.
 *
 *  === Asset key convention ===
 *  `"{theme}/{color}_{pieceType}_{state}"`, e.g. `"classic/white_king_idle"`.
 *  The theme segment (`"classic"`) is the one implemented theme for this phase.
 *  Future themes are introduced by adding a `theme` parameter or a lookup table
 *  here — no renderer changes are required.
 */
object VisualResolver:

  /** Resolve the [[VisualDescriptor]] for the given [[id]].
   *
   *  Always returns a valid descriptor.  Fallback behaviour (glyph rendering)
   *  is encapsulated in [[VisualDescriptor.fallbackSymbol]].
   */
