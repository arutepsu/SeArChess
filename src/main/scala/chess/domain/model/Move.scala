 ...
 *
 *  @param promotion  present only for pawn moves that reach the last rank; indicates
 *                    the piece type the pawn should be replaced with.  `None` means
 *                    the caller has not yet specified a promotion piece and the move
