...
object EnPassantValidator:
  ...
  def isEnPassantMove(board: Board, move: Move, enPassantState: Option[EnPassantState]): Boolean =
    enPassantState match
      case Some(ep) =>
        ...
        board.pieceAt(move.from).exists(p =>
          p.pieceType == PieceType.Pawn && p.color != ep.pawnColor
        )
      case None => false
   ...
   *
   *  Checks:
   *  - the capturing pawn advances exactly one file and one rank in the
   *    correct forward direction for its color
