 ...
 *  - structured [[NotationWarning]]s — non-fatal observations that did not
 *    prevent success (e.g. unknown tags, dropped extensions)
 *
 ...
 *
 *  There is intentionally no `target` field on this trait; the subtype is the
 *  target.
 */
...
object ImportResult:
   ...
   *
   *  @param data         the imported position, typed by the concrete importer
   *  @param sourceFormat the [[NotationFormat]] the position was parsed from
   *  @param metadata     position-specific import metadata
   *  @param warnings     non-fatal observations collected during import
   */
   ...
   *
   *  @param data         the imported game, typed by the concrete importer
   *  @param sourceFormat the [[NotationFormat]] the game was parsed from
   *  @param metadata     game-specific import metadata
   *  @param replay       optional replay summary describing the import
   *                      characteristics; absent when not applicable or not
   *                      computable
   *  @param warnings     non-fatal observations collected during import
   */
