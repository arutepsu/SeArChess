 ...
 *
 *  Warnings do not prevent a successful import but may indicate lossy
 *  translation, unsupported extensions, or normalisation that was applied
 *  silently.
 *
 *  Every variant exposes a human-readable [[message]].  Callers that need to
 *  branch on warning kind pattern-match on the subtype rather than parsing the
 *  message string.
 */
...
object NotationWarning:
   ...
   *
   *  @param name the unknown tag key (e.g. a PGN tag name)
   */
   ...
   *  @param reason why it was not mapped
   */
  ...
  /** An extension or annotation specific to a non-standard dialect was present
   *  but is not supported; it was silently dropped.
   *
   *  @param extension a name or description of the dropped extension
   */
  ...
  /** The importer silently normalised part of the input to conform to a
   *  canonical form (e.g. promoted a short castling annotation, filled a
   *  missing result token).
   *
   *  @param description what was normalised and how
   */
   ...
   *
   *  Prefer using a specific subtype.  Use this only when no other variant
   *  applies and introducing a new variant is not warranted.
   *
   *  @param message the free-form warning text
   */
