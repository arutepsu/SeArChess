 ...
 *
 *  Operates on the structured model produced by [[FenParser]].
 *  All checks are pure functions; no board state or domain objects are created here.
 *
 *  Validation scope:
...
object FenSemanticValidator:
  ...
  private def validateKingCount(
    ...
    countPieces(ranks, color, FenData.PieceSymbol.King) match
      case 1 => Right(())
      case 0 =>
        Left(ValidationFailure.MissingRequired(fieldName,
          s"Exactly one $colorLabel king is required; none found"))
      case n =>
        Left(ValidationFailure.InvalidValue(fieldName, n.toString,
          s"Exactly one $colorLabel king is required; found $n"))

  // ── Castling rights ──────────────────────────────────────────────────────────

  /** Castling rights are only consistent when the king and the corresponding
   *  rook are both on their original squares.  Standard FEN uses fixed home
   *  squares; Chess960 / X-FEN are out of scope for this validator.
   */
  private def validateCastlingRights(
    ...
    for
      _ <- if castling.whiteKingSide  then validateCastlingRight(ranks, "K", 4, 0, 7, 0, FenData.ActiveColor.White) else Right(())
      _ <- if castling.whiteQueenSide then validateCastlingRight(ranks, "Q", 4, 0, 0, 0, FenData.ActiveColor.White) else Right(())
      _ <- if castling.blackKingSide  then validateCastlingRight(ranks, "k", 4, 7, 7, 7, FenData.ActiveColor.Black) else Right(())
      _ <- if castling.blackQueenSide then validateCastlingRight(ranks, "q", 4, 7, 0, 7, FenData.ActiveColor.Black) else Right(())
    yield ()

  /** Verify that both the king and rook are on their required home squares for
   *  the given castling right.  King is checked first; only if the king is
   *  present is the rook square inspected.
   *
   *  @param kingFile  0-based file of the expected king square (always 4 = 'e')
   *  @param kingRank  0-based rank of the expected king square (0 = rank 1, 7 = rank 8)
   *  @param rookFile  0-based file of the expected rook square (0 = 'a', 7 = 'h')
   *  @param rookRank  0-based rank of the expected rook square
   */
  private def validateCastlingRight(
    ...
    if !isOccupiedBy(ranks, kingFile, kingRank, color, FenData.PieceSymbol.King) then
      Left(ValidationFailure.InvalidValue("castlingRights", rightLabel,
   ...
   *
   *  - When White is to move, the previous move was Black's two-square pawn
   *    advance, so the target must be on rank 6 (0-based rank 5 = '6').
   *  - When Black is to move, the previous move was White's two-square pawn
   *    advance, so the target must be on rank 3 (0-based rank 2 = '3').
   *
  ...
  private def validateEnPassant(
    ...
    ep match
      ...
      case FenData.EnPassantTarget.Square(_, rank) =>
        ...
        else Left(ValidationFailure.InvalidValue(
          "enPassant.rank",
          (rank + 1).toString,
   ...
   *
   *  FenData ranks are stored from FEN rank 8 (index 0) to rank 1 (index 7),
   *  so board rank `r` (0 = rank '1') maps to `ranks(7 - r)`.
   */
