 ...
 *  - standard initial position only (games started with a non-standard position
 *    cannot be exported and return [[ExportFailure.UnsupportedExportFormat]])
...
private[pgn] object PgnExporter extends NotationExporter[GameState]:
  ...
  override def exportNotation(data: GameState, format: NotationFormat): Either[NotationFailure, ExportResult] =
    format match
      case NotationFormat.PGN => exportAsPgn(data)
      case other =>
        Left(ExportFailure.UnsupportedExportFormat(other, s"PgnExporter does not support format: $other"))
  ...
  /** Replay `state.moveHistory` from the standard initial position, collecting
   *  one SAN token per move.  Fails on the first move that cannot be rendered.
   */
  private def renderMoveHistory(state: GameState): Either[NotationFailure, Vector[String]] =
    state.moveHistory.foldLeft[Either[NotationFailure, (GameState, Vector[String])]](
      Right((GameStateFactory.initial(), Vector.empty))
    ) {
      case (Left(err), _) => Left(err)
      case (Right((current, tokens)), move) =>
        SanRenderer.render(current, move).flatMap { san =>
          GameStateRules.applyMove(current, move).left.map { err =>
  ...
  /** Format SAN tokens as `1. e4 e5 2. Nf3 Nc6 ...`
   *  White's move starts with a move number; Black's move follows without one.
   */
  private def formatMoveText(tokens: Vector[String]): String =
    tokens.zipWithIndex.map { case (san, i) =>
