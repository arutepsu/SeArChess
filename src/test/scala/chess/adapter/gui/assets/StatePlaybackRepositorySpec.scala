...
class StatePlaybackRepositorySpec extends AnyFlatSpec with Matchers:
  ...
  private def singleSegMeta(state: VisualState, assetKey: String, mode: PlaybackMode = PlaybackMode.Clamp) =
  ...
  private val testRepo = StatePlaybackRepository(Map(
    ...
    "classic/white_pawn_attack" -> twoSegMeta(VisualState.Attack, "classic/white_pawn_attack", "classic/white_pawn_attack1", PlaybackMode.Loop),
  ...
  it should "return the correct state for a registered entry" in {
    testRepo.lookup(Color.White, PieceType.Pawn, VisualState.Idle).get.state shouldBe VisualState.Idle
  }

  it should "return the correct mode for a Clamp entry" in {
    testRepo.lookup(Color.White, PieceType.Pawn, VisualState.Move).get.mode shouldBe PlaybackMode.Clamp
  }

  it should "return the correct mode for a Loop entry" in {
    testRepo.lookup(Color.White, PieceType.Pawn, VisualState.Attack).get.mode shouldBe PlaybackMode.Loop
  }

  it should "return two segments for the Attack entry" in {
    val meta = testRepo.lookup(Color.White, PieceType.Pawn, VisualState.Attack).get
  ...
  it should "use the primary asset key derived by VisualResolver" in {
    ...
    val expected = VisualResolver.resolve(PieceVisualId(Color.White, PieceType.Pawn, VisualState.Move)).assetKey
  ...
  "StatePlaybackRepository.fromCatalog" should "build a single-segment Clamp entry from a Move entry" in {
    val entry   = StatePlaybackEntry(VisualState.Move, PlaybackMode.Clamp, Seq("classic/white_pawn_move"))
    ...
    val meta    = repo.lookup(Color.White, PieceType.Pawn, VisualState.Move).get
  ...
  it should "build a two-segment Loop entry from an Attack entry" in {
    val entry   = StatePlaybackEntry(VisualState.Attack, PlaybackMode.Loop, Seq("classic/white_pawn_attack", "classic/white_pawn_attack1"))
    val catalog = SpriteCatalog("t", Map.empty, Map("classic/white_pawn_attack" -> entry))
    val repo    = StatePlaybackRepository.fromCatalog(catalog)
    val meta    = repo.lookup(Color.White, PieceType.Pawn, VisualState.Attack).get
  ...
  it should "convert segment strings to PlaybackSegmentRef values" in {
    val entry   = StatePlaybackEntry(VisualState.Dead, PlaybackMode.Clamp, Seq("classic/black_queen_dead"))
    val catalog = SpriteCatalog("t", Map.empty, Map("classic/black_queen_dead" -> entry))
    val repo    = StatePlaybackRepository.fromCatalog(catalog)
    val meta    = repo.lookup(Color.Black, PieceType.Queen, VisualState.Dead).get
  ...
  it should "produce an empty repository from an empty catalog" in {
    ...
    StatePlaybackRepository.fromCatalog(catalog)
