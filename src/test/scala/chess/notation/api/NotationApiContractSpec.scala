...
class NotationApiContractSpec extends AnyFlatSpec with Matchers with EitherValues with OptionValues:
  ...
  "NotationFormat" should "enumerate all expected formats" in {
    val formats: Set[NotationFormat] = Set(NotationFormat.PGN, NotationFormat.FEN, NotationFormat.JSON)
  ...
  "ParsedNotationKind" should "enumerate all four variants" in {
    val kinds: Set[ParsedNotationKind] =
      Set(ParsedNotationKind.Fen, ParsedNotationKind.Pgn,
          ParsedNotationKind.JsonPosition, ParsedNotationKind.JsonGame)
  ...
  "ParseFailure" should "be a NotationFailure" in {
    val f: NotationFailure = ParseFailure.SyntaxError("unexpected token", line = Some(3), column = Some(7))
  ...
  it should "record optional dialect and version" in {
    val m = PositionImportMetadata(normalized = true, sourceDialect = Some("Chess960"), sourceVersion = Some("1.0"))
    ...
    def importNotation(
      ...
      (parsed, target) match
        case (fen: ParsedNotation.ParsedFen, ImportTarget.PositionTarget) =>
          Right(ImportResult.PositionImportResult(
  ...
  private object StubFacade extends NotationFacade[String]:
    def parse(format: NotationFormat, input: String): Either[ParseFailure, ParsedNotation] =
      format match
        ...
        case NotationFormat.PGN  => Left(ParseFailure.StructuralError("PGN parser not yet implemented"))
        case NotationFormat.JSON => Left(ParseFailure.StructuralError("JSON parser not yet implemented"))
